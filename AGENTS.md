...
TeXRA targets both VS Code and Electron. Follow these rules to keep business logic platform-agnostic:

1. **Never import `vscode` in VS Code-free zones.** See CLAUDE.md "Platform Decoupling" for the full list. The key ones: `src/agent/`, `src/model/`, `src/latex/`, `src/tools/`, `src/shared/`.
