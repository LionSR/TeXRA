  ...
  app
    ...
    .then(async () => {
      ...
      try {
        ...
        reopenMainWindow();
        // Off the ready path: the deferred cleanup reads the whole storage
        // root, so it starts after the first window is up and nothing awaits
        // it. Process shutdown cancels it if it has not started.
        processResources.add(scheduleDeferredSessionCleanup(processSession));
