...
const initVscodePlatform = Effect.fn('initVscodePlatform')(function* (
  ...
  // matching `apiKeyStatusBarItem`. Registering them here too would
...
const activateWorkspace = Effect.fn('activateWorkspace')(function* (
  ...
  const safeRefreshApiKeyStatus = Effect.suspend(apiKeyStatusRefresh).pipe(
    Effect.catchCause((cause) =>
      Effect.logError(
  ...
  yield* Effect.forkDetach(safeRefreshApiKeyStatus);
  ...
  onTexraAuthSessionsChanged(context, () => {
    runtime.runFork(safeRefreshApiKeyStatus);
  });
    ...
    vscode.commands.registerCommand('texra.refreshApiKeyStatus', () =>
      runtime.runPromise(
        apiKeyStatusRefresh().pipe(
          // Credential facts changed (set/unset API key from any entry
          // point — palette, walkthrough, welcome card), so the onboarding
          // funnel must recompute too: the State 0 card has no other signal
          // when a key is added outside the main view's own round-trip.
          Effect.andThen(
            progressViewProvider
