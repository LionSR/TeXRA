...
const miniMaxDetection = (envelope: z.infer<typeof MiniMaxEnvelopeSchema>) => ({
    ...
    Stream.suspend(() => {
      ...
      return Stream.unwrap(
        Effect.gen(function* () {
          ...
          let miniMaxName: string | undefined;
              ...
              Effect.gen(function* () {
                ...
                if (turn.protocol === 'minimax-chat') {
                  ...
                  if (!envelope.success)
                    return yield* new ModelError({
                      kind: 'malformed-output',
                      message: 'MiniMax returned malformed stream metadata.',
                  ...
                  const rejection = miniMaxFailure(
                    ...
                    {
                      ...miniMaxEvidence,
                      ...miniMaxDetection(envelope.data),
                    },
                  );
                  ...
                  if (chunk.usage != null) {
                    ...
                    miniMaxReceipt = {
                      ...
                      prompt_tokens_details: {
                        ...miniMaxReceipt?.prompt_tokens_details,
                        ...receipt.prompt_tokens_details,
                      },
                      completion_tokens_details: {
                        ...miniMaxReceipt?.completion_tokens_details,
                        ...receipt.completion_tokens_details,
                      },
                    };
                    usage = miniMaxUsage(miniMaxReceipt);
                  }
                } else if (chunk.usage) {
                  // The protocol-specific schema above requires complete principal counts.
                  usage = yield* normalizeUsage(
                ...
                let miniMaxReasoningDelta: string | undefined;
                if (turn.protocol === 'minimax-chat') {
                  const delta = choice.delta;
                  miniMaxContentSeen ||= delta.content != null;
                  if ('name' in delta && delta.name !== undefined) {
                    if (miniMaxName !== undefined && miniMaxName !== delta.name)
                      return yield* new ModelError({
                        kind: 'malformed-output',
                        message: 'MiniMax changed its assistant name.',
                      });
                    miniMaxName = delta.name;
                  }
                  if (
                    'audio_content' in delta &&
                    delta.audio_content !== undefined
                  )
                    miniMaxAudio = delta.audio_content;
                  if (
                    ...
                    for (const [
                      position,
                      detail,
                    ] of delta.reasoning_details.entries()) {
                      const index = detail.index ?? position;
                      if (index < 0 || index > miniMaxDetails.length)
                        return yield* new ModelError({
                          kind: 'malformed-output',
                          message:
                            'MiniMax returned an unsupported reasoning detail order.',
                        });
                      const previous = miniMaxDetails[index];
                      for (const key of [
                        'type',
                        'id',
                        'format',
                        ...
                        if (
                          ...
                          return yield* new ModelError({
                            kind: 'malformed-output',
                            message:
                              'MiniMax changed a reasoning detail identity.',
                          });
                      }
                      miniMaxDetails[index] = {
                        ...previous,
                        ...detail,
                      ...
                      miniMaxReasoningDelta += detail.text ?? '';
                    }
                  }
                }
                if (
          ...
          const completion = Stream.fromEffect(
            Effect.gen(function* () {
              ...
              if (
                turn.protocol === 'minimax-chat' &&
                (reasoning !== undefined || miniMaxDetails !== undefined)
              ) {
                completedContent.push({
                  ...
                  evidence: {
                    kind: 'minimax-reasoning',
                    ...(reasoning !== undefined ? { plain: reasoning } : {}),
                    ...(miniMaxDetails !== undefined
                      ? { details: miniMaxDetails }
                      : {}),
                  },
                });
