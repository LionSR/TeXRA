...
3. **`/resume` slash command** — opens a structured form (per [§ Slash command forms](../10-architecture.md#slash-command-forms)) listing recent sessions.
...
- **Numbered selection** `1.`–`9.` per [§ Intuitiveness conventions](../10-architecture.md#intuitiveness-conventions) — digit shortcuts jump directly.
...
- **Concurrent edit.** If the same session is also open in the VS Code extension, both hosts read the same log; appends are sequential through the shared store. (Detached subagents within a session are **not** re-attached — see [§ 8 multi-agent](../10-architecture.md#8-multi-agent-specifics).)
