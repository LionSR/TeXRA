...
- **Footer keymap** inside the palette is **mandatory** — every modal/form carries this strip to make "what do I do next?" answerable without leaving the screen. See [§ Intuitiveness conventions](../10-architecture.md#intuitiveness-conventions) for the cross-modal rule.

## Backing data

- **Slash commands** read from `tui/commands/slashRegistry.ts`. Each command declares: `name`, `aliases`, `summary`, optional `formComponent` (see [09-slash-form.md](./09-slash-form.md) for the structured-form variant).
...
- **Attachments** read from `cliState.attachments` — a `Map<id, AttachmentRef>` populated by paste detection (per [§ Image attachments](../10-architecture.md#image-attachments)). Selecting `[Image #N]` inserts the token at the input cursor; the model receives the actual image payload at send time.
