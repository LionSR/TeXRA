...
```
╭─ TeXRA ── agent: writer  ·  model: claude-opus-4.7  ─── 4 turns · $0.06 · 03:11 ────╮
│                                                                                      │
│  ◇ writer                                                                            │
...
- **Action row** uses bracketed `[ y ]` form for the keys. Mouse / arrow nav is **not** supported in modals (intentional per [§ Non-goals](../00-overview.md#4-non-goals-explicitly-excluded) — keyboard only).
