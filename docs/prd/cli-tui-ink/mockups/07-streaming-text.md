...
User ran `texra chat | tee transcript.txt`. stdin and stderr are still TTY so the input prompt and approval modals work normally — but stdout is piped, so Ink chrome is replaced with a plain ANSI serializer. This mode is auto-detected (no flag); `--print/-p` continues to take the full headless path (per [§ 13](../20-implementation.md#13-headless--legacy-preserved)).
