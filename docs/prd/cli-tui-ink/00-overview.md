...
The headless path (`texra run`, `--print/-p`, `--output-format json|ndjson`, non-TTY, CI) is preserved byte-for-byte. The new TUI runs only when the chat command is invoked on an interactive terminal; when stdout is piped while stdin/stderr remain TTYs, chrome degrades to plain streaming text rather than refusing to run — see [20-implementation.md § Headless](./20-implementation.md#headless--legacy-preserved).
