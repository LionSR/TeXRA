...
> posture (#7099) as the *long-term direction* — while keeping its lesson as a
> standing gate (no package without an already-enforced import boundary).
> Grounded in the 2026-07-09 macroscopic review
> ([`state-of-the-architecture-2026-07.md`](./state-of-the-architecture-2026-07.md),
...
> [`agent-sdk-readiness.md`](./agent-sdk-readiness.md) (the earlier
...
*environment boundary*, not the run surface:
...
- The **empirical SDK surface already exists**: exactly **15 `@agent/*`
  ...
  the surface *seed* — derived from use, not designed on paper.

## 2. The friction list (what an outside embedder trips on, ranked)

Verified against a paper embedder: *"load agent X from YAML, run on files
...
1. **The bootstrap incantation** (NS-1, strategic — *the* SDK tax). Reaching
...
2. **The per-run ceremony** (NS-3). A 265-LoC ordering-sensitive skeleton
   ...
   flush, toast bridge, shutdown status — runtime *persistence bookkeeping*
...
- Enforcement census: **one** boundary rule in the whole repo
  ...
  gate", `agent-sdk-readiness.md:148`) **was never built**.
...
*width* with a checked-in per-host baseline list (the repo already runs this
pattern: `knip-baseline.json`, `check-dead-code-ratchet.mjs`); new deep
specifiers require consciously extending the list.
*Trackers:* the never-built agent-sdk-readiness lint-gate step; #7152.
...
*Trackers:* #6968, #6890 (toolEdit channel), #6982/#6984 (sweeps), the #7636
micro-audit A2/A5/A6 rows. *New decision needed:* only D4's one-paragraph
trace-arm ruling.
...
lines *deleted* per concern (the #7560 train), explicitly **not** a
...
*Trackers:* #6966 Stage 3c persistence bullets; the readiness checkpoint's
runSession row *as the metric, not the wrapper*.
...
- **No `runSession()` facade / SDK wrapper layer** — the readiness doc's own
  ...
  the ceremony shrinks by *deleting* host-side bookkeeping into
