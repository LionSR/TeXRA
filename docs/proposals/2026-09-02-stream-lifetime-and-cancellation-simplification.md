...
| Candidate | Outcome                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A1        | **Landed.** `turnAbortController`, `startTurn`/`finishTurn`, and the controller parameter on `ChildRunStrategy.launch`/`runTurn` are gone; strategies take the loop signal. The Claude Agent SDK is the one boundary that wants a controller, so `runStreamedTurn` links one to the turn signal for exactly the stream it drives.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| A2        | **Landed, narrowed.** `ExecutionHandle.detachToolUseAbortListener` and the `signal` parameter of `attachToolUseFlow` are gone; `runToolUseFlow` subscribes to its own run signal for exactly its live window, and `ChildRunInterruptible` no longer reaches through the tracked handle into the flow (its `session`/`childStreamId` constructor parameters went with that hop). Two things stayed on evidence: the flow's own `interactions.cancel` is reached by the resume-at-attach and fallback paths without the lifecycle handler, so it is not a duplicate on every path; and the `interrupt()` fallback to the flow context (`ExecutionHandle.ts:90-94`) has a documented consumer, so it stays. `AgentRunLifecycle.vitest.ts` now pins the lifecycle's half of the contract (the signal carries a pre-attach stop) instead of the handle's deleted relay.                                                                                                                                                                                                                                                                                                                                                                 |
| A3        | Kept, as recorded.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| A4        | **Withdrawn.** `callController` has an independent producer: the `WorkflowScriptControl` skip/retry action aborts it directly (`runWorkflowScript.ts:390-400`), so the second `throwIfAborted` guards a real fact. The sweep missed that producer.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| B1        | **Withdrawn.** `SessionState.ts:92-100` already records the ruling that `_streamIncarnations` and `_removedStreams` "each have a genuinely different lifetime and stay as separate maps". Merging saves one map but not a lifetime; the ruling stands until someone overturns it on new evidence.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| B2        | **Landed.** The generation token is a field on `StreamRecord`; `ResidentStreamRegistry` keeps only presence and prune, and its stale "store-wide revision" sentence is gone.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| B3        | **Landed.** `StreamState.pins` replaces `leases` + `presentationLeases`; `requestEviction` is still the only site that names a member.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| B4, B5    | Unchanged, as recorded.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 2.C       | **Blocked on a maintainer ruling.** Every route to retiring a finished child's snapshot record — the applier calling the store, the CLI's focus-leave release calling it, or the transcript store's cold transition notifying it — needs exactly one new public method on `StreamSnapshotStore` (and a presentation-pin query on `StreamLogStore` for the applier route). `config/ratchets/store-public-surface-baseline.json` rejects growth in either store's public methods, and the standing rule is "never widen a baseline". The non-widening alternatives (self-retirement inside `attachSessionEvents` keyed on a resident-transcript proxy, or a change-feed flag on `StreamLogDelta`) either race the transcript's own flush or widen a wire contract. Recommendation: widen the baseline by one row, `StreamSnapshotStore.requestEviction`, with this section as the stated reason — the ratchet's own rationale (every public method is a caller-verified contract) is met by a method with two production callers — and keep the `warnIfUnseeded` gate as the regression detector. Until then, the CLI resume path remains the only production eviction and the record set grows with every stream a session touches. |
| 2.D, B4   | Blocked on the rulings named in §3.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| E1        | **Landed, narrowed.** The `onResult` missed-replay half is gone (map, two counters, predicate, microtask drain, option). `attachTerminalResultToast` keeps its `replayWhenAttached` option because the same flag also feeds the host presentation replay through `interactions.emit`, which is a different, tested mechanism.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
added; no new exports, ports, or ratchet rows.
