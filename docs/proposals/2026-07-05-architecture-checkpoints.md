...
| `readLegacyInstruction` (`StreamSnapshotStore.ts`; formerly `StreamTabStore.ts`, 145-LoC legacy per-run-instruction reader) + `LegacyInstructions*` schema support in `streamData.ts`                                           | Age-based, same persisted-run retention window as the `LegacyLogMessageSchema` row above (same #3061 refactor date): delete once persisted `legacyInstructions.json`/`runInstructions.json` sidecars predating #3061 are outside the supported persisted-run retention window; D3 verifies no supported pre-#3061 archival instruction files remain                                | executed 2026-08-02                           |
| `tool`→`toolName` transform (`src/shared/schemas/progressView/data.ts:48-67`)                                                                                                                                                   | Age-based: delete once persisted tool-use logs written before the `toolName` rename are outside the supported persisted-run retention window; D3 verifies no supported `tool`-only tool-use log entries remain                                                                                                                                                                     | executed 2026-08-02                           |
...
on 2026-08-02. The historical `propose_agent`, `propose_workflow`, and
