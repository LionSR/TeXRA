...
gone (that one arm now writes through the `UPDATE_STATE_SETTING` catalog path
— `SettingsProfileController.ts:124-129`; 18 other `globalState.update` sites
remain repo-wide), so schema validation and the
...
| `invalidate(streamId, slice)`                                                                                                             | substrate §6 / contracts §2.3 | REPLACEMENT — deletes 5 port methods × 2 impls and adds this one; port shrinks 22→18 (22 − 5 deleted + 1 added `invalidate`)(→~5 under projection-zero). The `slice` key type reuses the projection shape's existing field names, no new vocabulary.                                                                                                                                                                                   |
...
| Port 22→18: 4 payload-free methods + `onParentStreamChanged` (dead param) deleted, `invalidate` added, ×2 impls         | −60     | **V**                                                                                                                                                                                                                                          | §2.3 PR        |
...
6. **AppSignals in desktop/CLI** (V7) — **LANDED #10997 (2026-08-19)**: 4 signals wired on desktop, 2 fenced extension-only, 6/6 annotated with per-host consumption at the declaration (`AppSignals.ts`); the CLI stays at zero refs, now fenced deliberately on all six instead of ambiguous. Landed as the flagged net-add (+232 LoC, 96 of them the fence annotations), costed per surface in the PR — and the audit's "13 declared signals" premise was refuted at landing: 6 remain (`includedModelAccessChanged`, the six subscription signals, and `extensionDeactivating` (#10924) were already gone).
