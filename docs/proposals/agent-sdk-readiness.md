...
7d. _(in progress; long-horizon, behavior-sensitive)_ **PR 1 landed (2026-06-13)** — `SessionHandle` composition root + identity-wrapped `defaultSessionHandle` (wraps the four singletons by identity, zero call-site changes), `getActiveFlushers()` accessor in `@transcript`, `@texra/core` export, `SessionHandle.vitest.ts`; behavior-neutral, typecheck + vitest + eslint clean. PRs 2–7 pending: hosts create one per session and hand it to their resolve adapters — the actual multi-session-isolation payoff, and the natural place to later return a control handle from `runAgent`. Residue to fold in (audit §15): the `clearAll*` path and the one remaining module-level subscription; `executionSubscriptionBinder` joins the composed set. **Implementation design: [`session-handle-7d-design.md`](./session-handle-7d-design.md)** (PR-by-PR; PRs 1–5 behavior-neutral, 6–7 the two cutovers).
