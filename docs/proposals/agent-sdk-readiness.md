...
**Status:** Audit (2026-05-30; re-verified 2026-05-31 → 2026-06-06). **Steps 1–3 + 5 + 6 landed** (Step 4 folded into Step 5; Step 6 landed in PR #4781 / commit `da131dc`); Step 7 (a–d) pending. **SDK-008 fully closed** — `core/config.ts` passthrough removed (#5349), `core/stateStore.ts` `getGlobalState`/`getWorkspaceState` inlined to `platform()` (§14, 2026-06-06). See the re-verification addenda in [`../agent-sdk-readiness-audit.md`](../agent-sdk-readiness-audit.md) §9–§14 (latest: §14, 2026-06-06).
...
7a. `ToolUseAgentRegistry` Map → `SessionInterruptRegistry` class + module-default instance + delegators (zero call-site changes) + cross-instance isolation test. _Lowest risk; first._
7b. `executionRegistry` 3 Maps → `ExecutionRegistry` class; the module-level `StreamStatusService.onDidChange` becomes a constructor-wired instance subscription with `dispose()`. 71 call sites unchanged via delegators.
7c. `runCoordinators.bridgeState` (8 Maps + `retryCoordinatorRefs`) → `CoordinatorBridge` class; resolve-side exports stay delegators; the cross-key ref-counting + three-way fallback preserved verbatim. **Load-bearing — relocate, never delete.**
7d. _(long-horizon, behavior-sensitive)_ `SessionHandle` composing the three; hosts create one per session and hand it to their resolve adapters — the actual multi-session-isolation payoff, and the natural place to later return a control handle from `runAgent`.
