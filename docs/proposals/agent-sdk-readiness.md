...
**Status:** Audit (2026-05-30; re-verified 2026-05-31 → 2026-06-06; Step-7 status refreshed 2026-06-10). **Steps 1–3 + 5 + 6 landed** (Step 4 folded into Step 5; Step 6 landed in PR #4781 / commit `da131dc`); **Step 7a–c landed** (with name drift — tree has `InterruptRegistry` / `ExecutionRegistry` / `RunCoordinatorBridge`); **7d pending** (acceptance criteria below). **SDK-008 fully closed** — `core/config.ts` passthrough removed (#5349), `core/stateStore.ts` `getGlobalState`/`getWorkspaceState` inlined to `platform()` (§14, 2026-06-06). See the re-verification addenda in [`../agent-sdk-readiness-audit.md`](../agent-sdk-readiness-audit.md) §9–§14 (latest: §14, 2026-06-06).
...
4. ✅ **Folded into Step 5.** Standalone `runtime/`/`toolUse/` barrels were skipped — the audit verifier flagged them as "pure churn" without a lint gate, and `@texra/core` re-exports directly from source modules (it _is_ the barrel). The `core/index.ts` barrel was already deleted in Step 1.
5. ✅ **Populate `@texra/core` (LANDED):** replaced the one-line `corePackageReady` stub with the curated surface — `@platform` composition root, `AgentConfig`/`AgentCategory`, execution-request validation, the run entry (`executeAgent`/`runValidatedExecutionRequest`/`AgentFlowResult`), `AgentRuntimeHost`, the `AgentTrace` channel, the agent registry, and execution storage. Purely additive (deep `@agent/*` aliases still work; no consumer migration). The package typechecks with `types: ["node"]`, so a `vscode` leak into the surface now fails `workspace:typecheck` — a standing VS-Code-free guardrail. Removed `rootDir` from `packages/core/tsconfig.json` (meaningless under `noEmit`; it rejected the re-exported root files). **Deferred:** the `no-restricted-imports` lint rule — it can't distinguish new from existing code, so it would fire on ~287 existing deep imports; adoption/enforcement is a later incremental step.

### Steps 6–7 — refined 2026-05-30 (workflow re-analysis against current main)

A design-panel + adversarial-verify pass re-examined the original Steps 6–7 now that `@texra/core` exists. **Both structural designs were rejected as scoped** and the cleanest path is naming/curation, not restructuring:

- ❌ **Run-entry _facade_** (one `runAgent(config, options)` superset) hits a real type wall — `registerExecution` requires `AgentConfig` (parsed) but the direct callers (`DelegationTools.ts:365`, `runChatTui.tsx:865`) hold the looser `AgentConfigPayload`; `DelegationTools` uses a 6-arg register (parent/depth lineage) inexpressible via a boolean flag. And the self-explanatory "single entry" win is hypothetical: **zero files import `@texra/core` today**, so the "two siblings" confusion would just move from `index.ts` to `@agent/runtime`.
...
**Step 6 (refined) — ✅ LANDED (PR #4781, commit `da131dc`; re-verified in tree 2026-05-31).** Made the surface _say_ which entry is which (behavior-neutral). Renamed `runValidatedExecutionRequest` → **`runAgent`** (+ file `runExecutionRequest.ts` → `runAgent.ts`, `RunExecutionRequestOptions` → `RunAgentOptions`), body byte-identical. At `@texra/core`, alias `executeAgent` as **`runAgentStream`** (source untouched), add "START HERE" doc comments, drop `getAgentPath` from the surface, export `ExecutionId` / `ExecuteAgentOptions` / `WorkflowFlowResult`, number the export sections, add a top-of-file usage example + a `package.json` `exports` map. Updated the 4 referencing files **including the desktop dynamic-import string literal**, which was renamed from the old `@agent/runtime/runExecutionRequest` path to `@agent/runtime/runAgent` (not caught by `tsc`; no longer present in the tree post-rename). _Flag, don't fix:_ `setupAssistantCommand.ts:333`'s 3-arg `registerExecution` drops `agentCategory` — a real divergence, but fixing it is a behavior change, so leave it a documented follow-up.

**Step 7 (refined) — relocate module-global state incrementally (each PR behavior-neutral, classes with delegators):**

7a. ✅ **LANDED** (as **`InterruptRegistry`**, not the planned `SessionInterruptRegistry`) — `ToolUseAgentRegistry` Map → class + module-default instance + delegators (zero call-site changes) + cross-instance isolation test.
7b. ✅ **LANDED** — `executionRegistry` 3 Maps → `ExecutionRegistry` class; the module-level `StreamStatusService.onDidChange` became a constructor-wired instance subscription with `dispose()`. Call sites unchanged via delegators.
7c. ✅ **LANDED** (as **`RunCoordinatorBridge`**, not the planned `CoordinatorBridge`) — `runCoordinators.bridgeState` (8 Maps + `retryCoordinatorRefs`) → class; resolve-side exports stay delegators; the cross-key ref-counting + three-way fallback preserved verbatim. **Load-bearing — relocate, never delete.**
7d. _(pending; long-horizon, behavior-sensitive)_ `SessionHandle` composing the three; hosts create one per session and hand it to their resolve adapters — the actual multi-session-isolation payoff, and the natural place to later return a control handle from `runAgent`. Residue to fold in (audit §15): the `clearAll*` path and the one remaining module-level subscription; `executionSubscriptionBinder` joins the composed set.

**Step 7d acceptance criteria** (recorded 2026-06-10 by [`error-pipeline-and-ownership.md`](./error-pipeline-and-ownership.md) T3-1 — cited by event name + file basename, not line numbers, because line cites drifted twice during verification):

- **(a) One emission path for run-scoped events.** Core VS-Code-free zones emit run-scoped events only via the run's `runtimeHost`. The 7 direct `bus.emit` sites in `src/tools` (`goalStateChanged` in `goalStore.ts` — né `odysseyStore.ts` — and the inquiry events) re-route **only when the per-session handle exists**; re-routing earlier changes headless NDJSON output and risks in-process inquiry RPC resolution.
...
- **(c) Terminal result as data.** The `result` variant on `AgentEvent` (one discriminated arm emitted at the `runFlowWithLifecycle` boundary; `cancelled` a sibling of `failed`) is an acceptance criterion of the handle design — the handle is what hosts consume it from. See `error-pipeline-and-ownership.md` T3-2.
