...
**Status:** Audit (2026-05-30; re-verified 2026-05-31 → 2026-06-06; Step-7 status refreshed 2026-06-10). **Steps 1–3 + 5 + 6 landed** (Step 4 folded into Step 5; Step 6 landed in PR #4781 / commit `da131dc`); **Step 7a–c landed** (with name drift — tree has `InterruptRegistry` / `ExecutionRegistry` / `RunCoordinatorBridge`); **7d landed** (acceptance criteria below). **SDK-008 fully closed** — `core/config.ts` passthrough removed (#5349), `core/stateStore.ts` `getGlobalState`/`getWorkspaceState` inlined to `platform()` (§14, 2026-06-06). See the re-verification addenda in [`../agent-sdk-readiness-audit.md`](../agent-sdk-readiness-audit.md) §9–§14 (latest: §14, 2026-06-06).
