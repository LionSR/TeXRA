...
### C1 — Narrow `ResumeQueuedToolUseOptions` to its real surface *(strong)*
...
### C2 — Delete the two zero-passer injection ports on `RunReflectionFlowInput` *(strong)*
...
### C3 — Delete two dead fields on `ToolUseFlowContext` *(strong)*
...
### C4 — Delete `AgentRosterControllerDeps.fallbackTeamId` *(strong)*
...
### C5 — Make `AgentLaunchInput.executionId` required; delete the dead second mint *(strong)*
...
### C6 — One session default per chain *(correctness; net ~0 — flagged per the honesty bar)*
...
### C7 — Delete the redundant `interactions` parameter on `assembleAgentLaunchContext` *(solid)*
...
### C8 — Relocate `onFollowUpConsumed` off `SubagentRunOptions` *(solid)*
...
### C9 — Delete the dead defaulted ctor param on `SettingsViewMessageHandler` *(solid)*
...
### C11 — `DesktopAgentLaunchOptions` as a Pick *(cosmetic tail)*
...
### C12 — `userFollowUpSupport` derivation helper *(flagged: adds one element)*
...
### C13 — Approval trio: single owner for a launch-pinned fact *(design-level — demoted from mechanical by the adversarial pass)*
...
### C14 — Test-only seam demotion *(gated on the house ruling in the shared-contracts doc)*
