...
Items **1** and **15** are included in this PR (#9541). Completed rows remain below as an execution
record, not future work. Item 10 is split into landed dependency ownership (10a) and remaining
tsconfig/knip ownership (10b).
...
`test-kernel/` — 869 tracked files, 224,268 LOC, 57.4% of everything under `src/` by
...
**Current tally:** items 1-7, 10a, 11, and 15 are complete in the current PR-head tree. Items
2-7, 10a, and 11 came through merged PRs #9537/#9539; items 1 and 15 are part of this PR. Remaining
quick items are 13-14; remaining larger independent items are 8, 9, 10b, and 12.
...
**Completed waves — no future scheduling:** items 1-7, 10a, 11, and 15 are complete in this
PR-head tree through merged PRs #9537/#9539 and this PR. Do not carry them into new implementation
waves or rebudget their historical blast radii.

**Wave 1 — remaining free/small work.** Item 14 and any §4 prose not already incorporated into
`src/README.md` or `AGENTS.md`; audit the current text before editing so the completed guidance is
not duplicated.
...
| 18 = §5.C (`src/test-kernel` → root `test-kernel`) | 869 renames + 9 config files + 3 ratchet path rewrites; see below                                                                                             |
...
speculatively makes `@texra-ai/agent`'s declaration build emit for 869 test files);
...
per-package README work. Item 11's relocation landed in PR #9537; optionally consolidate or index the 21
checkpoint files now in `docs/dev/audits/`. None of this requires moving those files again.
...
7. **The one-Vitest-root convention.** Zero colocated tests anywhere in `src/**`
   ...
   "fix" test-kernel by scattering 869 files next to their sources.
...
- **Test quality and coverage were not assessed** — no judgment on whether 823
  suites over 166,365 LOC of production code is the right ratio, whether suites
