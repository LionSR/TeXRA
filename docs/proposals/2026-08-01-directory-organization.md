...
Status: partially executed and reconciled with the current branch.

Items **4-7** and **11** landed in `219b76272`; items **2-3** and the dependency-ownership half of
item **10** landed in `85793e66a8`; items **1** and **15** landed in `da36573927`. Completed rows
remain below as an execution record, not future work. Item 10 is split into landed dependency
ownership (10a) and remaining tsconfig/knip ownership (10b).
...
reviewed but not proven; the landed rows are the only ones exercised.
...
describes neither tests nor a kernel (`src/test-kernel/support/` is now 25 files).

The former indexing and written-rule defects are resolved. `src/README.md` now indexes all 19
production subsystems, bringing the tracked README count under `src/` from four to five. The
advanced branch also corrected the `src/utils/` browser-reachability rule and the
...
directory now has 53 Markdown files, zero readiness checkpoints, and the 21 checkpoint files live
under `docs/dev/audits/`. Ten table items or sub-items have landed; the remaining work is items 8,
9, 10b, and 12-14, plus the optional naming decisions — not a restructuring.
...
| 11  | ✅ **Landed in `219b76272`: archive the agent-SDK checkpoint series under `docs/dev/audits/`.**                                                                                                                                                                                                                                                                                                 | `docs/proposals/` now contains zero checkpoint files; all 21 checkpoints are in the archival directory.                                                                                                                                                                                                                                                                                                                                            | Relocation is complete. Optional future work is consolidation or an index, not another move.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | done   |
| 12  | **Fold the two doc singletons**: `docs/pocketflow/state_architecture.md` → `docs/architecture/pocketflow-state.md` (also fixes the only snake_case filename among 206 tracked docs markdown files); `docs/skills/skill-authoring.md` → `docs/dev/`                                                                                                                                              | Both directories hold exactly one file and CLAUDE.md links into them, so they are load-bearing but read as abandoned                                                                                                                                                                                                                                                                                                                               | **4 citations, not 2.** `CLAUDE.md:136` and `AGENTS.md:407` are gated by `scripts/check-guidance-refs.mjs` — a move without them **fails the commit**. `docs/README.md:23` and `docs/prds/cli-tui-ink/2026-05-14-30-reference.md:85` are _ungated and will rot silently_; `docs/README.md` in particular is the first file a stranger opens. Drop the now-empty `'pocketflow/**'` and `'skills/**'` entries from `publicDocs.js` `srcExclude`. **Caveat**: `state_architecture.md:35-40` documents `runState.toSnapshot()` / `AgentRunState.fromSnapshot()` against a codebase with no `AgentRunState` class (per `2026-08-01-open-source-readiness-audit.md`). Fix or delete the content _before_ promoting it into `architecture/`, or the rename makes a wrong doc more trusted                                                                                                                                                                                                                                                | 1 h    |
| 13  | **`git mv packages/extension/src/MainViewProvider.ts packages/extension/src/webview/`** — the directory name stays                                                                                                                                                                                                                                                                              | `progressView/` and `settingsView/` each hold their `*ViewProvider.ts` _and_ their `*ViewMessageHandler.ts`; the main view alone has its provider orphaned at the package src root (only 3 `.ts` files live there) while its handler sits in `webview/`. This fixes the actual asymmetry                                                                                                                                                           | **5 required path rewrites:** imports in `packages/extension/src/commands.ts:22` and `progressView/ProgressViewProvider.ts:44`; dynamic import in `src/test-kernel/webview/SidebarSurfaceOwnership.vitest.ts:91`; ratchet entry in `config/ratchets/shared-schemas-deep-import-baseline.json:505`; classifier path in `.claude/workflows/tech-debt-tournament.mjs:71`. **Explicitly does NOT rename the directory** — see §5, decision E, and §3. Run the sidebar suite, shared-schema ratchet, and tournament classifier smoke check                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 30 min |
...
| 15  | ✅ **Landed in `da36573927`: add `skills/` and `prompts/` to guidance-reference prefixes.**                                                                                                                                                                                                                                                                                                     | Guidance citations into both shipped trees are now checked.                                                                                                                                                                                                                                                                                                                                                                                        | No remaining action in this item.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | done   |

**Current tally:** items 1-7, 10a, 11, and 15 are landed. Remaining quick items are 13-14;
remaining larger independent items are 8, 9, 10b, and 12.
...
| **Split the remaining `docs/proposals/` into `active/` / `shipped/` / `archive/`**                                                          | The checkpoint archive already reduced the directory to 53 Markdown files. A further split still requires human classification and rewrites many ungated citations, including source comments. The landed item 11 has already delivered the low-risk reader benefit; reconsider only with a dedicated citation migration.                                                                                                                                                                                                                                                                                                                                                                                                             |
...
**Completed waves — no future scheduling:** items 1-7, 10a, 11, and 15 have landed. Do not
carry them into new implementation waves or rebudget their historical blast radii.

**Wave 1 — remaining free/small work.** Item 14 and any §4 prose not already incorporated into
`src/README.md` or `AGENTS.md`; audit the current text before editing so the landed guidance is not
duplicated.

**Wave 2 — remaining small atomic move.** Item 13 only. Its two imports and three hardcoded paths
...
Item 10b (trace-viewer generated path-map and knip ownership), item 12 (doc singletons), and any
per-package README work. Item 11's relocation is complete; optionally consolidate or index the 21
checkpoint files already in `docs/dev/audits/`. None of this requires moving those files again.
