...
| Four mainView types knip cannot see                   | `MainViewDroppedFileAttachmentPlan:20`, `MainViewDroppedFileAttachmentInput:28`, `MainViewExecutionLaunchResult:34`, `MainViewStartupControllerDeps:35` — each has only its declaration plus one self-reference repo-wide. Plus `desktopAgentExecution.ts:1198` `runExecution` public with 3 internal and 0 external callers. | 0 LoC, −5 elements                            | near-zero          |
...
(`OnboardingFunnelState` — every production consumer imports it from
`@shared/schemas`) and `ChatExportController.ts:28` (`ChatExportInput` — all
other consumers import from `@agent/export`).
