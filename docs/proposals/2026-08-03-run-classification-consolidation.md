...
| ----------------------------------------------- | ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `RunKind` `{agent, process, workflowScript}`    | `runDescriptor.ts:8-9`                                            | What owns the stream. The correct axis. The `RunKind` type is exported at `:9` and already consumed (`childStream.ts:17,28`); only `RunKindSchema` stays unexported, so nothing validates a kind at a boundary. |
...
| Persisted run kind (execution)      | `ExecutionMeta.runKind`                                | `registerExecution` only, at birth. **Today four of six call sites omit `category`.** Full inventory: omitted at `agentCliShared.ts:197`, `subagentExecution.ts:165`, `WorkflowScriptTool.ts:371`, `inBandSubagentExecution.ts:446`; passed at `bash.ts:415` (`'process'`) and `runAgent.ts:102` (`config.agentCategory`). That is why no execution on disk is ever tagged `workflowScript`. Becomes required. |
...
| `category === Workflow && entries.some(role === 'workflowTask')` (`App.tsx:310-313` and its byte-duplicate `panes/SubagentList.tsx:728-732`) | `identity.kind === 'workflowScript'`                                                        |
...
**Migration posture, explicitly: read-time healing, forward-only.** The legacy
arm at `streamSnapshotRead.ts:105-113` normalizes an old descriptor into
`RunIdentity` as it is read. There is no rewrite pass over
`streamData/*/meta.json`, so the affected rows stay in their old shape on disk
and are healed on every read until something rewrites them naturally. Chosen
over a migrate-on-disk pass because the descriptor is a derived, rebuildable
projection of `config.json`, not authored data, so a corrupt row costs a
degraded tab label rather than lost work. The consequence to accept is that the
legacy arm is permanent for as long as those buckets exist, and must therefore
be covered by a fixture test rather than treated as temporary scaffolding.
