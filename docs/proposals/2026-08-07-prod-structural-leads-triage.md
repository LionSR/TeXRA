...
- **selectFile() (dialogs.ts) is a one-line wrapper over selectFiles({...options, allowMany:false}) + paths?.[0] with a sole caller at fileSelectionCommands.ts:117 — inline and delete the export**
  evidence: packages/extension/src/frontend/ui/dialogs.ts:90-95 defines selectFile as `selectFiles({...options, allowMany: false})` then `paths?.[0] ?? null`; grep confirms exactly one prod caller (packages/exten
  fix: Owner: extension file-selection/dialogs. Inline into selectEditedFile (fileSelectionCommands.ts:115-122) as announceSelection(async () => (await selectFiles({ currentFile, openLabel: 'Select Edited File', filters: {} }))
