...
**one lint block copied from an existing recipe**, and no backend framework. The contract package
...
const NO_ACCOUNT: AccountPlane = Object.freeze({
  ...
  // signIn → frozen {kind:'failed'}; signOut → no-op; onChange → noop disposer
});
...
| `texra auth` subcommand tree (`packages/cli/src/commands/auth.ts`) and `texra setup-token` (`commands/relayTokens.ts`)                                                                                            | keep the public `texra auth` tree and `chatgptAuthCommand`; omit only TeXRA account/relay-token subcommands. The NDJSON kinds `'auth-status'`, `'relay-usage'`, `'relay-token'`, `'relay-token-info'`, `'relay-token-revoked'` (`src/schemas/cliOutput.ts:44-51`) stay in the union — removing them breaks a published contract; they are simply never emitted |
...
| **7** | **Gate the affordances** per §6.2: settings-tab predicate, LoginBanner, onboarding choice list, hosted CLI branches/options, walkthrough copy, telemetry default, delete `promoNotice.ts`; preserve shared ChatGPT/Kimi/public-auth commands                                                                                                                                                                                                                                                                                                                                                                                                                             | ~15 UI files + manifest                             | boot all three hosts without `installTexraBackend()`: zero TeXRA sign-in/upgrade surfaces or hosted-only options, while shared CLI auth/API commands still work                                                                      | after 4 (needs the atomic capability registry) |
