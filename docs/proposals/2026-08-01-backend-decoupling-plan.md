...
| `src/auth/{SupabaseClient,config,SupabaseSession,SupabaseAuthCoordinator,supabaseSessionTypes,TokenProvider,relayToken}.ts`, the Supabase token/code parsers and types currently in `src/auth/core/authCallback.ts`, and `src/auth/serverKeys/**`           | `config.ts:119-147` carries `UserTierSchema`, tier constants, `RELAY_TIER_SPENDING_LIMITS` — plan pricing with no public consumer after steps 0-3; callback parsing implements Supabase implicit-flow and PKCE protocols |
...
| `src/test-kernel/supabase/**` (4 files), `src/test-kernel/auth/EmailPolicy.vitest.ts`, the Supabase parser cases in `src/test-kernel/auth/authCallback.vitest.ts`, and the `relayCiToken` path assertion in `src/test-kernel/cli/RelayTokens.vitest.mts:81` | they exercise hosted policy, protocol parsing, or cross-import Deno source (`../../../supabase/functions/…`)                                                                                                             |
...
| `src/auth/codex/**` and a host-neutral callback-path module split out of `src/auth/core/authCallback.ts`   | third-party subscription plane plus only `getAuthCallbackBasePath` / `isAuthCallbackPath` and their routing tests; Supabase token/code parsers and parser tests move private   |
