...
| `src/auth/codex/**` and a host-neutral callback-path module split out of `src/auth/authCallback.ts`        | third-party subscription plane plus only `getAuthCallbackBasePath` / `isAuthCallbackPath` and their routing tests; Supabase token/code parsers and parser tests move private   |
