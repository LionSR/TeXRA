...
Actual at HEAD (8968988375), /Users/siruilu/Local/AI-Projects/coauthor/src/agent/core/state/AgentWorkspaceState.ts:
...
2. No barrel re-exports them. `rg "^export \*" src/agent packages/agent/src` returns nothing (no star barrels in src/agent at all). I read /Users/siruilu/Local/AI-Projects/coauthor/src/agent/followUp/index.ts (exports only describeFollowUpFailure, notifyFollowUpSent, presentFollowUpResult, submitFollowUp, SubmitFollowUpResult, FollowUpQueueInput, FollowUpRecoveryLease), /Users/siruilu/Local/AI-Projects/coauthor/src/agent/review/index.ts (collectReviewDiff, isPathInChangeSet, listBaseBranchCandidates, and the reviewIssues vocabulary), and /Users/siruilu/Local/AI-Projects/coauthor/packages/agent/src/index.ts. None of the ten appears. src/agent/roster/ has no index.ts.
3. Ratchets: none of the ten is in /Users/siruilu/Local/AI-Projects/coauthor/config/ratchets/knip-baseline.json (grep for all ten and for "followUp|roster|review" -> only an unrelated `toolHeaderPreviewBudget`). Un-exporting can only shrink, never widen, a baseline, so host-agent-import-baseline, shared-schemas-deep-import, host-agent-mock, and architecture-edges are all untouched.
4. Declaration-emit safety, the one way this could actually break: /Users/siruilu/Local/AI-Projects/coauthor/tsconfig.build.json runs `declaration: true, emitDeclarationOnly: true` over the packages/agent graph, which pulls in src/agent/followUp/*. Every one of the ten is referenced only from within its own module, so tsc emits the un-exported declaration into that module's own .d.ts. TS4023 ("cannot be named") only fires across modules, and no cross-module reference exists. Safe.
...
Single-producer confirmed: `rg -n 'log-usage'` shows exactly one POST path, /Users/siruilu/Local/AI-Projects/coauthor/src/telemetry/UsageLogService.ts:25, fed by /Users/siruilu/Local/AI-Projects/coauthor/src/agent/runtime/UsageMonitor.ts:262-275 (note: NOT src/agent/utils/ as the audit-era issues say). That call site emits 12 keys, neither field among them. The relay producer is gone (attic/supabase-relay). No parity fence exists — src/test-kernel/ has no supabase/ dir and nothing under it references usageValidation.
...
- **Inline hasPersistedParent and getPersistedUserFollowUpSupport into their one call site, reading meta.json once**
  ...
  (a) Consumer set is misreported. `rg -n -w` over the whole repo gives production hits only in `/Users/siruilu/Local/AI-Projects/coauthor/src/agent/runtime/executeAgent.ts` (imports at :16-17, call site :549-552, doc-comment references at :532 and :645), and FOUR test files — but not the ones claimed. `src/test-kernel/agent/runtime/RunAgentOwnership.vitest.ts` has zero hits (invented), while `/Users/siruilu/Local/AI-Projects/coauthor/src/test-kernel/agent/runtime/AgentLaunchActivation.vitest.ts:10-11,36-37,137,172` mocks BOTH symbols and was missed entirely.
  ...
  (d) The perf framing is overstated. `KVStoreCache` (`/Users/siruilu/Local/AI-Projects/coauthor/src/common/storage/KVStoreCache.ts`) caches handles not content, so the claim that two reads occur is technically right — but it is two concurrent reads of one small meta.json, once per tool-use resume. That is not "real work" worth a design argument.
