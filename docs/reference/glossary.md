...
This glossary provides definitions for technical terms used throughout the CoAuthor documentation. Reference this resource to understand specific terminology related to CoAuthor, AI models, LaTeX, and academic writing.

## CoAuthor-Specific Terms

### Agent

A specialized AI assistant in CoAuthor that performs a specific type of task, such as correcting errors, polishing writing, or creating figures. Each agent has custom prompts and settings optimized for its purpose.

### Chain of Thought (CoT)

An agent type in CoAuthor that uses structured XML output with scratchpad thinking for complex reasoning. CoT agents can show their work and reasoning process.

### Direct Agent

An agent type in CoAuthor that provides simpler, more straightforward output without intermediate reasoning steps. Direct agents are optimized for less complex tasks.
...
Settings that control how CoAuthor interacts with external tools and processes, including reflection, TeX counting, and prefilling options.
...
Instructions and context provided to an AI model to guide its response. In CoAuthor, prompts are carefully engineered for specific academic writing tasks.

### API (Application Programming Interface)

A set of rules that allows different software applications to communicate with each other. CoAuthor uses APIs to interact with language models from providers like Anthropic, OpenAI, and Google.
...
When an AI model generates information that is factually incorrect or not based on its input. CoAuthor's structured approach helps minimize hallucinations in academic writing.
...
Files generated by CoAuthor after processing input documents. These follow a naming pattern that includes the agent and model used.

### Working Directory

The main directory where CoAuthor operates, typically the root of your project or document collection.
...
Software suites for creating, editing, and converting images in various formats, used by CoAuthor for processing figures and diagrams.
