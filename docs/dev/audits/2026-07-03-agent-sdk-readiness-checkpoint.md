...
| Two logging idioms (`logUtils` vs `createChannelTrace`) are "redundant"                    | **Known / tracked** — run-scope vs module-singleton split targeted by `.agents/docs/archived/simplification/2026-05-17-logger-surface-cleanup.md`; the reader's "narrow `createChannelTrace` to 4 methods for log-only singletons" is the same tracked cleanup, not novel. |
