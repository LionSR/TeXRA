...
> **Status: 🟡 IN PROGRESS (2026-01-30)** - New audit round, no fixes applied yet
...
- **Location:** `STATUS_LABELS` map in `src/progressView/frontend/components/StreamHeader.ts:40-47`
...
- **Location:** `src/shared/styles/statusIndicatorStyles.ts:1-80` (no `.is-initializing` class exists)
...
- **Location:** `ENABLED_BUTTONS_BY_STATUS` map in `src/progressView/frontend/components/StreamHeader.ts:53-98`
...
- **Location:** `ProgressViewPrefsSchema` in `src/progressView/frontend/ProgressApp.ts:37-40`
...
- **Location:**
  - `ProgressViewPrefsSchema` in `src/progressView/frontend/ProgressApp.ts:37-40`
  - `sortStreams` / `streamComparators` in `src/shared/streams/streamSort.ts:26-41`
...
- **Location:**
  - `SET_FOLLOWUP_OPTIONS` handler in `src/progressView/frontend/messageDispatcher.ts:539-546`
  - `updateStreamContext` in `src/progressView/frontend/ProgressApp.ts:276-282`
...
- **Location:** `getActiveStreamInfo` in `src/progressView/frontend/ProgressApp.ts:248-258`
...
- **Location:** `UPDATE_INSTRUCTION` handler in `src/progressView/frontend/messageDispatcher.ts:388-402`
...
- **Location:**
  - `UPDATE_LOGS` / `APPEND_LOG` handlers in `src/progressView/frontend/messageDispatcher.ts:214-297`
  - `prependInstructionForToolUse` in `src/progressView/frontend/stateUtils.ts:119-145`
...
- **Location:** `renderFileList` remove spans in `src/webview/frontend/components/FileSelectGroup.ts:365-369`
...
- **Location:** `renderFileList` remove spans in `src/webview/frontend/components/OutputFilesSection.ts:120-124`
...
- **Location:** toggle-icon span in `src/webview/frontend/components/FileSelectGroup.ts:428-435`
...
- **Location:** toggle-icon span in `src/webview/frontend/components/OutputFilesSection.ts:139-146`
...
- **Location:** toolbar menu toggles in `src/webview/frontend/components/FileSelectGroup.ts:238-308`
...
- **Location:** chevron/toggle span in `src/webview/frontend/components/OutputFilesSection.ts:131-146`
...
- **Location:** `performSearch` / `applySearchToItems` in `src/historyView/frontend/components/HistoryList.ts:106-189`
...
- **Location:** `performNavigate` in `src/historyView/frontend/components/HistoryList.ts:117-127`
...
- **Location:** `categoryClass` normalization in `src/profileView/frontend/components/AgentsTable.ts:54-63`
...
- **Location:** `StreamTabInfoSchema` status field in `src/shared/schemas/stream.ts:49-62`
...
- **Location:**
  - `StreamStatusSchema` in `src/shared/schemas/stream.ts:16-24`
  - `STATUS_LABELS` / `ENABLED_BUTTONS_BY_STATUS` in `src/progressView/frontend/components/StreamHeader.ts:40-98`
...
| ----- | -------- | -------------------------------------------------------------------- |
| P1-P2 | MED      | `INITIALIZING` missing from `STATUS_LABELS` and styles               |
| P3    | HIGH     | `INITIALIZING` missing from `ENABLED_BUTTONS_BY_STATUS`              |
...
| M1-M6 | LOW/MED  | Interactive spans lack keyboard/ARIA affordances (pre-existing gaps) |
