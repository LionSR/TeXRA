...
- **Root cause:** `addPermission()` appends new permissions to the end of the array,
  but keyboard shortcuts always target `permissions[0]` (oldest).
- **Fix (root):** Either prepend new permissions or target the most recent entry
  (e.g., `permissions.at(-1)`), and document ordering expectations.
...
### M4) Multiple Outputs toggle is not keyboard accessible (LOW)

- **Area:** MainView
- **Type:** Accessibility regression
- **Impact:** Low (keyboard users cannot expand/collapse output list)
- **Root cause:** Toggle is a `<span>` with `@click` only.
- **Fix (root):** Use a `<button>` or add `role="button"`, `tabindex="0"`, and
  keydown handlers.
- **Location:** `src/webview/frontend/components/OutputFilesSection.ts`

### M5) Output file remove buttons are mouse-only (LOW)

- **Area:** MainView
- **Type:** Accessibility regression
- **Impact:** Low (keyboard users can’t remove individual files)
- **Root cause:** Remove control is a `<span>` with `@click` only.
- **Fix (root):** Use a `<button>` or add keyboard handlers and `tabindex`.
- **Location:** `src/webview/frontend/components/OutputFilesSection.ts`

### M6) File list toggle icon is not keyboard accessible (LOW)

- **Area:** MainView
- **Type:** Accessibility regression
- **Impact:** Low (keyboard users can’t expand/collapse multi-file lists)
- **Root cause:** Toggle is a `<span>` with `@click` only.
- **Fix (root):** Use a `<button>` or add `role="button"`, `tabindex`, and key handlers.
- **Location:** `src/webview/frontend/components/FileSelectGroup.ts`
...
| **M4**  | LOW      | ✅        | Toggle not keyboard accessible           |
| **M5**  | LOW      | ✅        | Remove buttons not keyboard accessible   |
| **M6**  | LOW      | ✅        | File list toggle not keyboard accessible |
