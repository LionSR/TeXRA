...
- **Stream Header Actions**:
  - <i class="codicon codicon-debug-stop"></i> **Stop**: Attempts to gracefully stop the currently running task for this stream. If the provider supports `AbortController` for streaming requests (such as OpenRouter), any ongoing streaming call is cancelled immediately; otherwise the current API call will finish first.
