...
1. **A proposal you approve.** Before anything runs, the lead's script is shown as a proposal labelled **Multi-agent workflow** with the script's name, its phases, the task list, the default agent and model, and the files bound to the run. It also carries a cost warning, because tasks run concurrently. You can approve, or reject with feedback so the lead adjusts its plan. The script itself is saved under `.texra/workflow-scripts/` in your workspace, so you can open it.
2. **Phases and per-agent progress.** The run opens as its own stream. Calls are grouped under the phases the script declared, and each task row shows its status: Planned, Running, Finished, Saved result (replayed from an earlier attempt), Skipped, Cancelled, or Failed. When a task finishes, its row shows the model, the elapsed time, and what it cost.
