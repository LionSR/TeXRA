...
A few things to notice. The `meta` block is the plan: because `tasks` is declared, the proposal and the progress view can show all three tasks before any of them run. `parallel()` runs the two fixes concurrently and waits for both. A failed call and a call you skipped both resolve to `null`, so the script filters nulls out before the merge; the run log says which calls were skipped. Each workflow-agent call resolves to a result that lists the files it produced, and those files can be handed straight to the next call.
...
| :--------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `export const meta`          | Required first statement. `name` and `description` are required. `phases` lists phase titles, `tasks` is the optional declared plan (`{ id, label, phase? }`), and `timeoutMs` sets the whole-run wall clock (1 second to 24 hours; default 60 minutes).                                                                                                                                                                         |
...
| `pipeline(items, ...stages)` | Runs each item through the stages on its own, with no waiting between stages, so one item can be verified while another is still being read. Each stage receives the previous result, the original item, and its index. A stage that returns `null` ends that item as `null`.                                                                                                                                                    |
...
Ordinary JavaScript handles the rest: a `for` loop with awaited calls runs stages in order, and array methods such as `.filter()` and `.map()` are how results fan back in. The session's child-run budget sets how many agent calls run at once, a single run makes at most 1000 live calls, and `parallel()` and `pipeline()` take at most 4096 items each. The script's own code gets 60 seconds of CPU in total; time spent waiting on agents does not count, so this only stops a loop that never awaits. `Date.now()` and `Math.random()` are unavailable so that a rerun replays the same call sequence.

The lead reads the bundled `multi-agent-orchestration` skill for how to shape a run: when to use `pipeline()` or a barrier, adversarial verification, referee panels, and sweeps that run until nothing new turns up.
...
**The run stopped with a wall-clock timeout.** The default limit for a whole run is 60 minutes, which a large fan-out on a slow model can exceed. Completed calls are journaled, so ask the lead to rerun the script with the same `meta.name`; it can raise `meta.timeoutMs` in the saved script (up to 24 hours) for the second attempt.

**"used its 60000ms guest CPU budget".** The script's own code, not an agent, ran for a minute in total. That almost always means a loop that never awaits an `agent()` call. Ask the lead to fix the loop in the saved script and rerun it with the same `meta.name`.
