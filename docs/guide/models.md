...
Claude Fable 5 (`fable5`) is Anthropic's most capable model. Thinking is always on — adaptive, with summarized reasoning — so there is no separate `T` variant. It supports the full reasoning-effort range up to Extra High, and is eligible for context compaction in tool-use mode.
