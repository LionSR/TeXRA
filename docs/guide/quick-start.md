...
2. Select the <wa-icon library="texra" name="wand"></wa-icon> **Auto-extract options** button to enable, for this run:
...
Press **Execute** (<wa-icon library="texra" name="play"></wa-icon>). The ProgressBoard opens in the TeXRA view and streams the run live. Read the [ProgressBoard guide](./progress-board.md) for help interpreting the logs.
...
3. Compare the original and modified versions with either of these:
   ...
   - **TeXRA's LaTeXdiff feature**: Use the LaTeXDiffs section in the TeXRA panel for a compiled, visual comparison. It creates a PDF with additions highlighted in blue and deletions in red.
...
For details on how LaTeX diff works, read the [LaTeX diff guide](./latex-diff.md).

## From the CLI

The same agents run from the terminal. After
`npm install -g @texra-ai/cli` (Node.js 22.19.0 or later in 22.x, or Node.js 24 or later) or
...
# Or let the guided setup handle credentials, environment, and your first task:
texra setup
...
After your first task with TeXRA, you can:
...
- Use [intelligent merge](./intelligent-merge.md) to combine changes
- Tune your workflow with [custom configuration](./configuration.md)

For issues or questions, read the [troubleshooting guide](/guide/troubleshooting) or check the [GitHub issues page](https://github.com/LionSR/TeXRA/issues).
