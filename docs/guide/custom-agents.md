...
> non-empty entry as the initial request and treats the remaining non-empty
> strings as reflection prompts. Empty strings (including those generated by
> templating) are ignored so later prompts still run. If a run requests more
> reflections than the list provides, the first reflection template is reused.
> Legacy `userReflect` blocks remain supported and are used whenever the array
> omits follow-up prompts.
