...
This guide will walk you through the process of installing CoAuthor and all its dependencies to ensure optimal performance.

## System Requirements

CoAuthor is designed to work on all major operating systems with the following minimum requirements:
...
CoAuthor relies on several external tools to function properly. Follow the instructions for your operating system.

### LaTeX Distribution

::: tip IMPORTANT
A LaTeX distribution is required for CoAuthor to properly process LaTeX documents.
:::
...
CoAuthor requires API keys to access language models. Here's how to set them up:

1. Open VS Code with CoAuthor installed
2. Click on the CoAuthor icon in the Activity Bar
3. Click "Set API Key" in the CoAuthor panel
...
To verify that CoAuthor and all dependencies are correctly installed:

1. Open VS Code
2. Click on the CoAuthor icon in the Activity Bar
3. The CoAuthor panel should load without errors
4. Create or open a LaTeX document
5. Try a simple command like "Indent Current TeX" from the editor title menu

If any component is missing, CoAuthor will typically show an error message indicating what needs to be installed.
...
With CoAuthor and all dependencies installed, you're ready to start using the tool to enhance your academic writing. Check out the [Quick Start Guide](/guide/quick-start) to learn the basics, or explore specific features in the other documentation sections.
