...
Need the accompanying references as well? Use the complementary `extract_bib_entries` tool to pull the BibTeX records for every citation found in the same LaTeX file. See the [Tools Reference](/reference/tools#extract_bib_entries) for parameter details.
