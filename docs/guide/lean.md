...
3. The Lean 4 extension installs **elan** and the Lean toolchain on first open.

TeXRA detects the running Lean 4 extension and routes its tools through it, so you share the same language server, diagnostics, and proof state you see in the editor.
...
If you don't have elan yet, follow the [Lean community install guide](https://leanprover-community.github.io/install/).
...
Describe the task in plain language; the agent decides which tools to call. Below are the five Lean tools the agent reaches for, as they surface in the tool-call log.
...
The `lean_file` tool runs file-scoped commands: `restart` (reload the language server for one file) and `refresh_dependencies` (a lighter refresh). Use these when results look out of date.
...
<p class="hero-caption">Server and build commands run everywhere; the Setup group drives the Lean 4 extension's installers, so it's gated to VS Code. <code>fetch_cache</code> pulls the whole project; <code>fetch_file_cache</code> covers only the current file's imports, which is faster.</p>

::: warning Setup commands are VS Code-only
The setup commands drive the Lean 4 extension's installers, so they only work in the VS Code build. In the CLI they fail with a "run the shell command directly" message. Manage your toolchain with `elan` and `lake` directly instead (for example `elan self update`, `elan toolchain install`, or `lake update`). Follow the [Lean install guide](https://leanprover-community.github.io/install/).
:::
