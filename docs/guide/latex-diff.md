...
To see latexdiff in action, try running the `polish` agent on the [sample document](/examples/draft.pdf). The agent produces colour-coded diffs showing exactly what changed between the original and each revision round.
