...
| Async boundary    | `Stream.toAsyncIterable(stream)`, `Stream.unwrap(effect)`, `Stream.interruptWhen(effect)`        | `effect`         | the SDK's `AgentRun.view` (10.3): Effect's own destructor, no runtime or fiber for the embedder; `interruptWhen` ends the iteration when the run settles |
