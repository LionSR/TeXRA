...
schema correctness, run-loop and event-channel invariants, webview lifecycle
issues, configuration and storage correctness, and CI/toolchain hygiene. Do not run
...
Conversely, flag pass-through layers this pull request newly introduces, per
...
load-bearing thin layers (dependency-injection seams, multi-caller DRY
helpers), which are thin by design rather than redundant.
