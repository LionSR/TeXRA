...
const DummyFigure = () => (
  <svg
    className="w-full h-48 bg-gray-100 rounded border border-gray-200"
    viewBox="0 0 300 200"
  >
    {/* Axes */}
    ...
    {/* Labels */}
    <text x="160" y="180" textAnchor="middle" fill="#374151">
      Bond dimension χ
    </text>
    <text
      x="30"
    ...
    <text x="250" y="30" textAnchor="end" fill="#dc2626" fontSize="12">
      Exact
    </text>
  </svg>
);
...
const DiffText = ({ text }) => {
  const parts = text.split(
    /((?:<diff-add>.*?<\/diff-add>)|(?:<diff-del>.*?<\/diff-del>))/g,
  );
  return (
    <>
      {parts.map((part, i) => {
    ...
    <div className="w-full max-w-6xl mx-auto p-8 bg-white rounded-lg shadow-lg">
      ...
      <div className="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
        ...
        <p className="text-blue-900">
          Update the figure caption to better describe the numerical convergence
          results and add comparison with exact diagonalization.
        </p>
      </div>
      ...
      <div className="mt-6 flex justify-between items-center p-4 bg-gray-50 rounded-lg">
        ...
        <div className="flex items-center gap-2">
          ...
          <div
            className={`h-2 w-12 rounded ${currentStep >= 2 ? "bg-blue-600" : "bg-gray-200"}`}
          ></div>
        </div>
      </div>
    </div>
