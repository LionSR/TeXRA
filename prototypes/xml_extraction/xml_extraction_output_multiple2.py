...
input_xml = "output_multiple2.xml"
...
tags_to_wrap = ["scratchpad", "document"]
...
for document in latex_documents.findall("document"):
    ...
    document_content = document.text.strip()

    print(f"\nLaTeX Document: {source}")
    cprint(document_content, "yellow")
    print("\n" + "=" * 50 + "\n")

    # Save the document content to a .tex file
    with open(source, "w") as tex_file:
        tex_file.write(document_content)
