...
- **New `resolve*`/`derive*`/`infer*`/`fallback*` over an already-decided fact needs the #6951 "Single source of truth" justification** in the PR body; if the fact exists upstream or can be carried as data, carry it. Precedents: `executionStreamResolver.ts` `resolvePersistedStreamIdForExecution` (hot-path reverse scan); the dual `resolveWorkspaceSourceDir` (compileCheck.ts vs LatexDiffManager.ts, divergent heuristics).
