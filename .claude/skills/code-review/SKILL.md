---
name: code-review
description: TeXRA-aware code review. Use for /review, PR audits, or any code review of this repo. Catches platform-coupling, Zod v4, run-loop, factory, modern-TypeScript, and webview-render rules that generic passes miss.
---
...
2. **Identify the change surface.** Run `git diff` (or `git diff main...HEAD`) and group changed files by zone: VS Code-free zones (`VSCODE_FREE_ZONE_DIRS` in `eslint.config.mjs`) vs. VS Code-allowed zones (`extension.ts`, `commands/`, `frontend/`, `common/webview/`, `auth/`, `platform/`).
3. **Check the event-channel rule.** New run-scoped facts extend `AgentEvent` (trace); facts the session authors extend the `SessionEvent` schema (`src/shared/schemas/sessionEvent.ts`). Flag any new `bus.emit` from a VS Code-free zone and any new subscribe surface. The `src/tools` emit sites this rule once grandfathered have migrated to session-owned emission (`SessionHandle.publish` in `src/agent/runtime/`), so a new direct `bus.emit` is a violation, not a grandfathered pattern. This does not restrict `appSignals.emit(...)` on the separate `AppSignals` bus (`src/eventBus/AppSignals.ts`) within its documented scope. Background: `.agents/docs/implemented/architecture/2026-06-10-error-pipeline-and-ownership.md`.
