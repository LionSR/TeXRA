...
intptr_t cleanup_root_handle(const cleanup_root *root) { return root->fd; }
...
int cleanup_loaded_module_path(char **path, cleanup_error *error) {
  static const char module_anchor = 0;
  Dl_info module;
  *path = NULL;
  if (dladdr(&module_anchor, &module) == 0)
    return fail(error, EIO);
  *path = strdup(module.dli_fname);
  return *path == NULL ? fail(error, ENOMEM) : 0;
}
