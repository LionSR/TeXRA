...
int cleanup_loaded_module_path(char **path, cleanup_error *error) {
  ...
  *path = NULL;
  ...
  WCHAR *wide = malloc((size_t)capacity * sizeof(WCHAR));
  if (wide == NULL)
  ...
  *path = malloc((size_t)bytes);
  if (*path == NULL) {
    free(wide);
    return fail(error, ERROR_NOT_ENOUGH_MEMORY);
  }
  if (!WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, wide, -1, *path,
                           bytes, NULL, NULL)) {
    number = GetLastError();
    free(*path);
    *path = NULL;
  ...
  free(wide);
  return number == ERROR_SUCCESS ? 0 : fail(error, number);
}
...
intptr_t cleanup_root_handle(const cleanup_root *root) {
