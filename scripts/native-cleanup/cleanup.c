...
/* Session graph construction is an existing synchronous host boundary, as is
 * the SQLite open beside it. Only the unbounded traversal uses async work. */
...
static napi_value loaded_module_path(napi_env env, napi_callback_info info) {
  (void)info;
  char *path;
  cleanup_error error;
  if (cleanup_loaded_module_path(&path, &error) != 0) {
    napi_throw(env, native_error(env, &error));
    return NULL;
  }
  napi_value value;
  napi_status status = napi_create_string_utf8(env, path, NAPI_AUTO_LENGTH, &value);
  free(path);
  if (status != napi_ok)
    return NULL;
  return value;
...
NAPI_MODULE_INIT() {
  napi_property_descriptor methods[] = {
      {"loadedModulePath", NULL, loaded_module_path, NULL, NULL, NULL, napi_default, NULL},
  ...
  napi_define_properties(env, exports, sizeof(methods) / sizeof(methods[0]),
                         methods);
  if (rooted_sqlite_exports(env, exports) != napi_ok)
    return NULL;
  return exports;
}
