...
- **Numerics teammate and "Teams" rebranding for Multi-Agent** — added a dedicated *Numerics* teammate (computes, plots, and writes about numerical results from LaTeX), and renamed the Multi-Agent tab to **Teams** to match how agents are actually organized. Existing teams keep their members; the change is visual only.
