...
- **Experimental: use your own ChatGPT subscription for Codex models** — sign in with your ChatGPT Plus/Pro/Team account and turn on "prefer ChatGPT subscription" to run OpenAI's Codex models on your included plan instead of a separate API key. Sign in from the terminal with `texra auth chatgpt login`, which opens your browser or, on a headless shell, shows a one-time device code; check or end the session with `texra auth chatgpt status` and `logout`. Off by default and clearly marked experimental and personal-use: it relies on an unofficial OpenAI endpoint that can change or stop working without notice, and it only ever uses your own signed-in session.
