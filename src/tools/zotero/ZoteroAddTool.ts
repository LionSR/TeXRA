...
interface ZoteroConnectorResponse {
...
async function isZoteroRunning(port: number): Promise<boolean> {
  try {
    const response = await axios.get(`http://127.0.0.1:${port}/connector/ping`, {
      timeout: 2000,
    });
...
export class ZoteroAddTool extends defineTool({
  ...
  protected async execute({ items, collection }: ZoteroAddInput) {
    ...
    for (const item of items) {
      ...
      try {
        // If DOI is provided, use Zotero's translation server to fetch metadata
        if (item.doi) {
          const saveResponse = await axios.post<ZoteroConnectorResponse>(
            `http://127.0.0.1:${port}/connector/saveDOI`,
            {
              doi: item.doi,
              ...(collection && { targetID: collection }),
            },
            {
              timeout: 30000,
          ...
          if (saveResponse.status === 200 || saveResponse.status === 201) {
            results.push({ item: itemLabel, status: 'success' });
          } else {
            results.push({
              item: itemLabel,
              status: 'error',
              message: `Unexpected response status: ${saveResponse.status}`,
            });
          }
        } else if (item.url) {
          // Use URL-based saving
          const saveResponse = await axios.post<ZoteroConnectorResponse>(
            `http://127.0.0.1:${port}/connector/saveSnapshot`,
            {
              url: item.url,
              ...(collection && { targetID: collection }),
            },
