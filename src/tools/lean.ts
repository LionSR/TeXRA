...
export class LeanGoalTool extends defineTool({
  name: 'lean_goal',
  description: `Get the proof goal state at a specific location in a Lean 4 file.

This is useful for understanding what needs to be proved at a particular point in a proof.

Usage:
- Specify the file and line number
- Optionally specify column for more precision

The tool inserts a temporary \`sorry\` at the specified location to extract the goal state,
then removes it. This gives you the exact proof context and goals at that point.

Note: This modifies the file temporarily. Make sure the file is saved.`,
  schema: LeanGoalInputSchema,
}) {
  protected async execute(input: LeanGoalInput): Promise<ToolResult> {
    const { file, line, cwd } = input;

    // For now, we'll just run lean on the file and look for goal-related info
    // A more sophisticated approach would insert `sorry` at the position
    // and parse the resulting goal state
