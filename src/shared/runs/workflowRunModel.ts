...
export interface WorkflowRunModel {
  ...
  readonly childRunOf: ReadonlyMap<string, RunId>;
  /** A card's child run progress, by row id, where the card has a child
   *  stream of its own (`childRunOf`) and the host holds that stream. */
...
interface WorkflowRunModelInput {
  ...
  /** Live progress by child stream, for the cards that opened those runs. */
  readonly childProgress: ReadonlyMap<RunId, ChildRunProgress>;
}
...
export function workflowRunModel(
  ...
  const claimants = new Map<RunId, WorkflowTaskRow | null>();
  for (const row of tasks) {
    const childRunId = row.call.childRunId;
    if (childRunId === undefined) continue;
    claimants.set(childRunId, claimants.has(childRunId) ? null : row);
  }
  const childRunOf = new Map<string, RunId>();
  const liveOf = new Map<string, ChildRunProgress>();
  for (const [childRunId, row] of claimants) {
    if (!row) continue;
    childRunOf.set(row.id, childRunId);
    const progress = input.childProgress.get(childRunId);
