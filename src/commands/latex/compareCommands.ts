...
async function handleCompare(
  ...
  try {
    ...
    const baseUri = vscode.Uri.file(toAbsolutePath(fileToUse));
    const editedUri = vscode.Uri.file(toAbsolutePath(editedFile));

    // Verify both files exist
    if (!(await existsFlexible(fileToUse))) {
      vscode.window.showErrorMessage(`Base file not found: ${fileToUse}`);
      return;
    }

    if (!(await existsFlexible(editedFile))) {
...
async function handleAcceptEdited(
  ...
  try {
    ...
    const editedContent = await readFlexible(editedFile);
    ...
    const targetPath = toAbsolutePath(fileToUse);
    await writeFlexible(targetPath, editedContent);
