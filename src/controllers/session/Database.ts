...
> =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      // Charge the encoded SQLite row before decoding or retaining it. The
      // optional display policy does not constrain the canonical owner reads.
      const readEvents = (
        ...
        for (const row of rows) {
          if (budget) {
            bytes += sessionMessageBytes(row);
            if (bytes > budget.bytes || events.length >= budget.rows)
      ...
      return {
        ...
        readAll: (fromCommit, throughCommit, budget) =>
          query(() =>
            readEvents(
              all.iterate(fromCommit, throughCommit ?? currentCommit()),
              budget,
            ),
          ),
        readListing: (budget) =>
          query(() =>
            readEvents(listing.iterate(JSON.stringify(LISTING_TYPES)), budget),
          ),
        readAggregate: (id, fromSeq, budget) =>
          query(() => readEvents(aggregate.iterate(id, fromSeq), budget)),
        aggregatesAfterCommit: (ids, afterCommit, throughCommit, budget) =>
          query(() =>
            readEvents(
              after.iterate(
                JSON.stringify(ids),
        ...
        readInputBatch: (ids, fromCommit, checkedIds = ids, budget) =>
          transaction(
            'read',
            () => {
