 ...
 *
 * Under the write transaction, this layer validates, redacts, and serializes
 ...
 *
 * The native connection and cleanup share a physically held storage root.
...
import * as nativeStorage from '@agent/storage/nativeSessionStorage.mjs';
...
import { makeSqliteClient, readTransaction } from './SqliteClient';
...
> =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      // life of the process.
      const cleanupRoot = yield* RcRef.make({
        acquire: Effect.acquireRelease(
          Effect.try({
            try: () =>
              nativeStorage.openRoot(realpathSync.native(roots.storage)),
            catch: ensureError,
          }),
          (root) => Effect.sync(() => nativeStorage.closeRoot(root)),
        ),
      ...
      const native = yield* Effect.gen(function* () {
        if (mode === 'ephemeral')
          return yield* Effect.acquireRelease(
            Effect.try({
              try: () => nativeStorage.openMemoryDatabase(),
              catch: openFailed,
            }),
            (connection) => Effect.sync(() => connection.close()),
          );
        yield* Effect.try({
          try: () => mkdirSync(roots.storage, { recursive: true }),
          catch: openFailed,
        });
        const root = yield* RcRef.get(cleanupRoot).pipe(
          Effect.mapError(openFailed),
        );
        return yield* Effect.acquireRelease(
          Effect.try({
            try: () => nativeStorage.openDatabase(root, SESSION_DATABASE_FILE),
            catch: openFailed,
          }),
          (connection) => Effect.sync(() => connection.close()),
        );
      ...
      const sql = yield* makeSqliteClient(native, (connection) =>
        Effect.gen(function* () {
          const committed = commitFromRows(
            yield* connection.executeUnprepared(highWater, [], undefined),
          );
      ...
      const transaction = <A, E>(
        ...
        (mode === 'read'
          ? readTransaction(sql, body)
          : sql.withTransaction(body)
        ).pipe(mapDatabaseFailure(failed));
      ...
      return {
        ...
        cleanupGeneratedDirectories: (ids) =>
          Effect.scoped(
            RcRef.get(cleanupRoot).pipe(
              Effect.flatMap((root) =>
                Effect.tryPromise({
                  try: () =>
                    nativeStorage.removeExecutionDirectories(
                      root,
...
/**
 * Bring the scoped native connection to the state C1 requires.
 *
...
function verifyPragma(
  ...
  const row = db.execute(`PRAGMA ${pragma}`, []).rows[0];
