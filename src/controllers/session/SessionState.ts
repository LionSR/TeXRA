 ...
 * together (`clearEphemeralStreamState`), so they share one map entry rather
 * than two separately-keyed maps; `execution` is still optional because a
 * stream can accrue metadata (e.g. a description set before RUNNING) before
 * `getOrCreateStreamState` ever mints it.
 *
 * `_streamMetadataCache` (a derived read cache invalidated by comparing
 * `metadata`/summary/timestamp identity) and `_removedStreams` (tombstones,
 * which must outlive the very ephemeral state they retire) each have a
 * genuinely different lifetime and stay as separate maps.
 */
...
export class SessionState {
  ...
  >();
  /**
   * Identities removed this session. The fact applier refuses any later fact
   * naming a removed stream, so a stale status, roster, edge, or attachment
   * fact cannot re-mint the transcript, execution, or metadata state deletion
   * dropped. This set is the single owner of that rejection, and a removal
   * is final: a stream id names one run and is never reused (decision 9), so
   * nothing re-claims an entry.
   *
   ...
   * finality claim. An entry lives for the session, the proven horizon for
   * in-flight facts.
   */
  ...
  /**
   * Begin a removal barrier for `stream`. Explicit ownership: `created` is
   * true only when this call installed a new barrier. When one already exists
   * (a `removeStream` fact racing a direct delete, or a command racing a
   * fact-path removal) the caller must not start a second deletion or retire
   * a barrier it does not own.
   *
  ...
  beginStreamRemoval(stream: StreamTabId): {
    ...
    if (this._removedStreams.has(stream)) {
      return { created: false, changedRosterParents: [] };
    }
  ...
  /**
   * Drop the tombstone of a deletion that retained the stream. Canonical
   * rosters already contain the newest authoritative rows, so retiring the
   * projection barrier reveals them in their original order.
   */
  retireStreamTombstone(stream: StreamTabId): {
    retired: boolean;
    changedRosterParents: StreamTabId[];
  } {
    if (!this._removedStreams.has(stream)) {
  ...
  /**
   * Finalize the tombstone and scrub the stream's canonical roster rows.
   * Returns the parents whose host-facing projection must be refreshed.
   */
  commitStreamTombstone(stream: StreamTabId): {
    committed: boolean;
    changedRosterParents: StreamTabId[];
  } {
    if (!this._removedStreams.has(stream)) {
