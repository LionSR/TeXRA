...
import { normalizeOpenRouterUsage } from '@agent/modelHandlers/openrouter/openRouterUsage';
...
const price = (raw: ChatUsage): number =>
  normalizeOpenRouterUsage(raw, 0, 'openrouter', CONFIG).cost;
...
describe('OpenRouter usage pricing', () => {
  ...
  it('preserves a native zero cost', () => {
    expect(price(usage({ cost: 0 }))).toBe(0);
  });
  ...
  it('falls back to static pricing when native cost is absent', () => {
    expect(price(usage())).toBeCloseTo(STATIC_PRICE, 12);
  });
});
