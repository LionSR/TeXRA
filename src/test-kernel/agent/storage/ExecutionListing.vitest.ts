...
describe('execution listing normalization', () => {
  ...
  it('lists only readable execution metadata with an explicit stream reference', async () => {
    ...
    const legacy = 'f9892002' as ExecutionId;
    const secondLegacy = 'f9892003' as ExecutionId;
    const ambiguous = 'f9892004' as ExecutionId;
    const malformed = 'f9892005' as ExecutionId;
    await getExecutionStore(referenced).writeMeta({
      timestamp: '2026-08-08T00:00:00.000Z',
      streamId: 'referenced-stream',
    });
    for (const executionId of [legacy, secondLegacy, ambiguous]) {
      await getExecutionStore(executionId).writeMeta({
        timestamp: '2026-07-31T00:00:00.000Z',
      });
    }
    const sidecars: ReadonlyArray<readonly [StreamTabId, ExecutionId]> = [
      ['legacy-stream', legacy],
      ['second-legacy-stream', secondLegacy],
      ['ambiguous-a', ambiguous],
      ['ambiguous-b', ambiguous],
    ];
    for (const [streamId, executionId] of sidecars) {
    ...
    try {
      ...
      expect(listing.references).toEqual([
        ...
        { executionId: legacy, streamId: 'legacy-stream' },
        { executionId: secondLegacy, streamId: 'second-legacy-stream' },
      ]);
      // Ambiguous compatibility evidence remains unreadable and is never
      // converted into a runtime stream edge.
      expect([...listing.unreadable.keys()]).toEqual([ambiguous]);
      await expect(getExecutionStore(legacy).readMeta()).resolves.toMatchObject(
        { streamId: 'legacy-stream' },
      );
      expect(
        (await getExecutionStore(ambiguous).readMeta())?.streamId,
      ).toBeUndefined();
      // One storage-owned listing shares both compatibility scans across all
      // legacy rows, while modern rows do not trigger their own scans.
      expect(
      ...
      const withMalformed = await listExecutionStreamReferences();
      expect(new Set(withMalformed.unreadable.keys())).toEqual(
        new Set([ambiguous, malformed]),
      );
