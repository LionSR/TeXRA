...
describe('native OpenAI Chat protocol', () => {
  it.effect(
    'retains incremental MiniMax reasoning, repeated text, calls and partial receipts for replay',
    () =>
      Effect.gen(function* () {
        const detail = {
          index: 0,
        ...
        const frames = [
          ...
          chunk({
            choices: [
              {
                index: 0,
                delta: { reasoning_details: [{ index: 0, text: 'again' }] },
                finish_reason: null,
              },
            ],
          }),
          ...
          chunk({
            choices: [
              { index: 0, delta: { content: 'same' }, finish_reason: null },
            ],
          }),
          toolChunk([
          ...
          chunk({
            choices: [],
            usage: { prompt_tokens: 0, total_characters: 0 },
        ...
        expect(result.content).toEqual([
          {
            ...
            evidence: {
              ...
              plain: '',
              details: [{ ...detail, text: 'againagain' }],
            },
          },
          {
            kind: 'message',
            content: [{ kind: 'text', text: 'samesame' }],
            evidence: {
        ...
        expect(result).toMatchObject({
          finishEvidence: { kind: 'minimax', outputSensitive: false },
        });
        ...
        expect(sent.messages[1]).toEqual({
          ...
          content: 'samesame',
          reasoning_content: '',
          reasoning_details: [{ ...detail, text: 'againagain' }],
          tool_calls: [
            {
              id: 'call_0',
              type: 'function',
  ...
  it.effect(
    'interrupts incremental MiniMax before joining reader cleanup',
    () =>
      Effect.gen(function* () {
        const reached = yield* Deferred.make<void>();
        let signal: AbortSignal | null | undefined;
        const cleanupFailure = new Error('MiniMax cleanup failed');
        const cancel = vi.fn(() => {
        ...
        const fiber = yield* model.streamTurn(turn).pipe(
  ...
  it.effect.each([
          ...
          choices: [
            {
              index: 0,
              delta: {
                reasoning_details: [{ index: 0, id: 'first', text: 'a' }],
              },
              finish_reason: null,
            },
          ],
          ...
          choices: [
            {
              index: 0,
              delta: {
                reasoning_details: [{ index: 0, id: 'second', text: 'b' }],
              },
              finish_reason: 'stop',
            },
          ],
    ...
    },
    {
      ...
      frames: [
        toolChunk([{ index: 0, function: { arguments: '{}' } }], 'tool_calls'),
      ],
      kind: 'malformed-output',
    },
  ])(
    'rejects unsafe MiniMax stream completion: $name',
    ({ frames, sentinel, kind }) =>
      Effect.gen(function* () {
        let body = sse(...frames);
        if (sentinel === false) body = body.replace('data: [DONE]\n\n', '');
        const fetch = vi.fn<typeof globalThis.fetch>(async () =>
          response(body),
        );
        const model = openaiChatModel(
