...
describe('SupabaseAuth PKCE flow state', () => {
  ...
  it.effect('persists only the flow state, never the session slot', () =>
    Effect.gen(function* () {
      ...
      // never appear here.
      const keys = yield* secrets.listStoredKeys();
  ...
  it.effect(
    'completes a callback delivered to a different client instance',
    () =>
      Effect.gen(function* () {
        const secrets = new FakeSecrets();
        yield* Effect.promise(() => startSignIn(createAuth(secrets)));
        ...
        const { data, error } = yield* Effect.promise(() =>
          secondWindow.client.auth.exchangeCodeForSession('auth-code'),
        );
        ...
        const remaining = yield* secrets.listStoredKeys();
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const { data, error } = yield* Effect.promise(() =>
          auth.client.auth.exchangeCodeForSession('auth-code'),
        );
