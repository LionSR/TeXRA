...
describe('the C1 event table and the C6 publisher', () => {
  ...
  it.effect(
    'fails only budgeted readers before retaining an oversized public replay',
    () => {
      const storage = workspace();
      return Effect.gen(function* () {
        const db = yield* Database;
        const committed = yield* db.appendAll([runStart, waiting]);
        const decode = vi.spyOn(SessionEventSchema, 'parse');
        for (const budget of [
          { bytes: 1, rows: 100 },
          { bytes: 1_000_000, rows: 1 },
        ]) {
          const reads = [
            db.readAll(0, undefined, budget).pipe(Effect.asVoid),
            db.readListing(budget).pipe(Effect.asVoid),
            db
              .readAggregate(runStart.aggregateId, 0, budget)
              .pipe(Effect.asVoid),
            db
              .aggregatesAfterCommit(
              ...
              .pipe(Effect.asVoid),
            db
              .readInputBatch([runStart.aggregateId], 0, undefined, budget)
              .pipe(Effect.asVoid),
          ];
          for (const read of reads) {
            decode.mockClear();
            const failure = yield* Effect.flip(read);
            expect(failure.cause).toBeInstanceOf(SessionReaderError);
            expect(decode).toHaveBeenCalledTimes(budget.bytes === 1 ? 0 : 1);
          }
        }
        decode.mockRestore();
        // The canonical owner is unrestricted, and overload never removes data.
        expect(yield* db.readAll(0)).toEqual(committed);
        expect(
          yield* db.readInputBatch([runStart.aggregateId], 0),
        ).toMatchObject({
          cursor: committed.at(-1)?.commit,
          events: committed,
        });
      }).pipe(Effect.provide(substrate(storage)));
    },
