...
- **Diagnostics are `Effect.log*`, not the `@logger/logUtils` writers.** A
  program names its channel once with `withLogChannel` from
  `@logger/effectLog` and every entry below it inherits that channel, so no
  helper takes a channel parameter just to log. Both producers end at the
  same host sink, so a test asserts on the entries the sink received
  (`@test/support/logSinkCapture`) rather than on a logger-namespace spy. One
  consequence to know: the Effect logger drops `Debug` entries unless
  `texra.logger.debugMode` is on, where the `@logger/logUtils` writers emit
  them and let the host's level filter decide. `LatexMediaManager`'s
