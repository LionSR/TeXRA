...
const CHANNEL = 'workspaceFileManager';
...
export class WorkspaceFileManager {
  ...
  public static getFullPathFromWorkspace(filePath: string): string {
    const workspacePath = this.getWorkspacePath();
    if (!workspacePath) {
  ...
  public static async readFile(filePath: string): Promise<string> {
    const fullPath = this.getFullPathFromWorkspace(filePath);
  ...
  public static async moveFile(
    ...
    try {
      const fullSourcePath = this.getFullPathFromWorkspace(source);
      const fullDestPath = this.getFullPathFromWorkspace(destination);
  ...
  public static async createDirectory(relativePath: string): Promise<void> {
    try {
      const fullPath = this.getFullPathFromWorkspace(relativePath);
  ...
  public static async readDirectory(
    ...
    try {
      const fullPath = this.getFullPathFromWorkspace(dirPath);
  ...
  public static async fileExistsAndNonTrivial(
    filePath: string,
  ): Promise<boolean> {
    return (
  ...
  public static async filterExistingFiles<T extends { path: string }>(
    ...
    const fileCheckPromises = items.map(async (item) => {
      try {
        ...
        const exists = await this.fileExists(item.path);
        return { item, exists };
      } catch (error) {
