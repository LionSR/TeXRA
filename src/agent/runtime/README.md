...
| **Session, event hub & emission**               | The per-session event contract and its single emit path to hosts  | `AgentRuntimeHost` (the progress-sink interface every host implements), `SessionHandle` (one composition record per session), `SessionEventHub` (typed session facts), `LegacyProgressEventProjection` (facts → legacy host events), `sessionProgressEventProjection` (session/run facts → progress payloads), `emitRuntimeEvent` (single emit path, SDK Step 7d F-1), `runFactEvents` (run-fact domain event names), `conversationProgressHub`, `StreamStatusService` (stream status state machine), `streamTab` (stream tab id), `terminalResultToast`, `ProgressViewBridge` |
| **Run context & user-interaction coordinators** | Ambient per-run context and session-scoped user-interaction gates | `RunContext` (`AsyncLocalStorage`-based ambient context), `HostInteractions` (session-owned host interaction port), `runCoordinators` (accessors over `RunContext` + `executionRegistry`), `AgentProposalCoordinator`, `PlanApprovalCoordinator`, `RetryRequestCoordinator`                                                                                                                                                                                                                                                                                                    |
