...
| **Run registry & live handles**      | Track, interrupt, and tear down in-flight runs                   | `RunHandle` (the live handle type plus run-owned interrupt capability), `runRegistry` (the session-facing surface: admission, registration, lookup and the stop gestures), `runRoster` (one entry per run — handle, child activation, parked fiber, lifecycle lane — the single in-process liveness authority), `runStopping` (what a stop does with those records), `detachSubagentsOnStop` (detach-vs-cascade policy)                                                                                                                               |
