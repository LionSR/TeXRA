...
/**
 * Increment the per-Odyssey accounting counters after a turn finishes.
 * No-op when there is no Odyssey on the stream or when the record is in
 * a terminal state (complete / abandoned).
 *
 * Safe to call unconditionally from the tool-use cycle's progress sink —
 * the store handles missing records and clamps negative deltas.
 */
