  ...
  continuation: {
    template:
      [
        ...
        '  remaining work.',
        '',
        'Only call `odyssey(complete)` when current evidence proves every',
        'requirement is satisfied; cite that evidence in `reason`.',
        'Otherwise keep working in scoped checkpoints. Self-pause via',
        '`odyssey(pause)` only when you genuinely need user input to',
        'proceed.',
        '</odyssey_context>',
      ].join('\n') + '\n',
  },
  objective_updated: {
    template:
      [
        ...
        'only served the previous one. Do not call `odyssey(complete)`',
        'unless the updated objective is actually complete, with current',
        'evidence proving every requirement is satisfied.',
        '</odyssey_context>',
      ].join('\n') + '\n',
  },
};
