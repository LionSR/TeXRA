 ...
 * explicit trace (tests, embedded callers that don't care about the SDK
 * surface).
 */
...
class NoopStreamHandle implements StreamHandle {
  ...
  finalize(_finalText?: string): void {}
}

export const noopTrace: AgentTrace = {
  emit: NOOP,
  subscribe: (_sub: AgentTraceSubscriber) => NOOP,
  debug: NOOP,
  info: NOOP,
  warn: NOOP,
  error: NOOP,
  ...
  openStream(_kind, options?: StreamOptions) {
