...
| `google/`     | `modelHandlerGoogleGenAI` (chat / `generateContent`), `modelHandlerGoogleInteractions` (Interactions API, flag-gated via `texra.model.useGoogleInteractionsAPI`, default on — additive sibling that resends the verbatim `Step[]` each round), plus Google helpers (`googleUsage`, `googleInteractionsUsage`, `googleMessageHelpers`, `googleSdkError`)                                                                                                         |
