...
| `openai/`     | `modelHandlerOpenAI`, `modelHandlerOpenAIResponse`, Responses-API collaborators (`OpenAIResponseWebSocketTransport`, `ResponseStreamProcessor`, `responseStreamEvents`, `openAIResponseContent`, `openAIResponseFileUploads`), OpenAI helpers (`openAIMessageUtils`, `openAIResponseErrors`, `BaseReasoningStreamAggregator`), and the OpenAI-**compatible** providers that extend `ModelHandlerOpenAI` (`modelHandlerDashScope/DeepSeek/GLM/Kimi/MiniMax/XAI`) |
| `google/`     | `modelHandlerGoogleGenAI` (chat / `generateContent`), `modelHandlerGoogleInteractions` (Interactions API, flag-gated via `texra.model.useGoogleInteractionsAPI`, default off — additive sibling that resends the verbatim `Step[]` each round), plus Google helpers (`googleUsage`, `googleInteractionsUsage`, `googleMessageHelpers`, `googleSdkError`)                                                                                                            |
