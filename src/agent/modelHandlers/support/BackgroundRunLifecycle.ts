...
import { BackgroundPoller, type BackgroundPollStats } from './BackgroundPoller';
 ...
 */
interface BackgroundRunLifecycleConfig<TClient, TResponse, TRetrieveParams> {
...
export class BackgroundRunLifecycle<TClient, TResponse, TRetrieveParams> {
  ...
  private async pollToTerminal(
    ...
    const verdict = this.config.createTerminalVerdict(
      polled,
      pollId,
      pollStats,
    );
