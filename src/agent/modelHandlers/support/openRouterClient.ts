...
export async function createOpenRouterClient(
  ...
  let module: typeof import('@openrouter/sdk');
  try {
    module = await import('@openrouter/sdk');
  } catch (err) {
    throw new Error(
      'Failed to load @openrouter/sdk. Run "npm install" to ensure the dependency is available.',
    );
  }
