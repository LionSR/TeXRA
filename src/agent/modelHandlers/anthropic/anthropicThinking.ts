 ...
 * Mythos-class models (Fable 5, Mythos 5) reject sampling parameters
 * (temperature/top_p/top_k) outright, so they belong here too.
 */
 ...
 * Per Anthropic docs, Opus 4.6, Opus 4.7, Opus 4.8, Sonnet 4.6, and the
 * Mythos-class models support adaptive thinking. Opus 4.7+ and Mythos-class
 * only accept adaptive thinking — manual budget_tokens returns 400.
 */
...
export function mapAnthropicEffort(
  ...
  switch (reasoningEffort) {
    case 'max':
      // The top tier ("Max"). Opus 4.8/4.7/4.6 and Mythos-class models all
      // accept Anthropic's 'max' effort; everything else caps at 'high'.
      return isClaudeOpus48(fullName) ||
        ...
        : 'high';
    case 'xhigh':
      // Opus 4.8 and the Mythos-class models expose the distinct 'xhigh'
      // ("extra") effort tier the SDK added in 0.100.0, which is exactly what
      // TeXRA's "Extra High" selector means — map to it directly. Opus 4.6/4.7
      // predate the tier split and only accept 'max'; everything else caps at
      // 'high'.
      if (isClaudeOpus48(fullName) || isMythosClass(fullName)) return 'xhigh';
...
export function buildThinkingConfig({
  ...
  if (supportsAdaptiveThinking(fullName)) {
    ...
    // Request 'summarized' so thinking still streams to the user — older
    // adaptive-thinking models already emit reasoning by default and are
    // unaffected.
