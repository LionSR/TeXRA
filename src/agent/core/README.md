...
| `usage/`      | Usage value objects              | `RunUsageAccumulator` — accumulates already-normalized `NormalizedUsage` (`@agent/types/NormalizedUsage`). Raw per-provider usage payloads (`ProviderUsage`) remain inside model handlers and live in `@agent/types/ProviderUsage`; core never imports them.                                                                                                                                                                                  |
