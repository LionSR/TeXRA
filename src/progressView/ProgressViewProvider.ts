...
export class ProgressViewProvider implements vscode.WebviewViewProvider {
  ...
  private _outputFiles: Map<string, string[]> = new Map();
  ...
  public addOutputFiles(stream: string, files: string[]): void {
    this._outputFiles.set(stream, files);
    this._saveState();
    if (this._view && stream === this._activeStream) {
      this._view.webview.postMessage({
        command: COMMANDS.UPDATE_FILES,
  ...
  public getOutputFiles(stream: string): string[] | undefined {
