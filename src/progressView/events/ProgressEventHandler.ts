...
export class ProgressEventHandler {
  ...
  private ensureActiveRunId(stream: StreamTabId): string | null {
    ...
    if (this.shouldUseDefaultRun(stream)) {
      this.state.setActiveRunId(stream, DEFAULT_RUN_ID);
  ...
  private shouldUseDefaultRun(stream: StreamTabId): boolean {
    const taskState = this.state.getTaskState(stream);
    const category =
      taskState?.session?.agentCategory ??
      this.state.getSessionKindHint(stream);
    if (category === AgentCategory.ToolUse) {
      return true;
    }

    const files = this.state.outputFiles.getFiles(stream);
    if (files.has(DEFAULT_RUN_ID)) {
      return true;
    }

    const usage = this.state.usageStats.getRunUsage(stream);
    if (usage.has(DEFAULT_RUN_ID)) {
      return true;
    }

    const instructions = this.state.runInstructions.getInstructions(stream);
    return instructions.has(DEFAULT_RUN_ID);
  }
