 ...
 */
export interface StreamStatusCallbacks {
...
/**
 * Register stream status event handlers.
 */
export function registerStreamStatusEvents(
  bus: ProgressEventBusLike,
  state: ProgressViewState,
  updater: WebviewUpdater,
  callbacks: StreamStatusCallbacks,
): vscode.Disposable[] {
        ...
        withEventErrorHandling(
          ...
          async () => {
            ...
            if (updater.isAvailable()) {
              callbacks.replayPendingTaskGroups(stream, updater);
            }

            const status: StreamStatus =
              callbacks.streamStatus.get(stream) ?? STREAM_STATUS.RUNNING;
            ...
            if (updater.isAvailable()) {
              ...
              updater.updateAll(state, callbacks.streamStatus);
            }

            callbacks.setStreamStatus(stream, status);

            if (updater.isAvailable()) {
              // Only force rebuild when actually switching streams.
              // Use the returned runId to avoid duplicate resolveRunId call.
              const activeRunId = callbacks.refreshStreamSurface(stream, {
                updateInstruction: false,
                forceRebuild: isStreamSwitch,
              });
              callbacks.sendInstructionUpdate(stream, activeRunId);
            }
          },
        );
      }),
    ...
    new vscode.Disposable(
      bus.on('updateStreamStatus', (payload) => {
        withEventErrorHandling(
          ...
          () => callbacks.setStreamStatus(payload.stream, payload.status),
        );
      }),
    ),
    new vscode.Disposable(
      bus.on('setTaskState', (data) => {
        withEventErrorHandling(MODULE, 'failed to handle setTaskState', () => {
          ...
          if (state.activeStream === streamTabId) {
            callbacks.sendInstructionUpdate(streamTabId);
          }

          if (updater.isAvailable()) {
            const infos: StreamTabInfo[] = buildStreamInfos(
              state,
              callbacks.streamStatus,
