...
> {
  ...
  async addFiles(
    ...
    for (const [round, files] of Object.entries(filesByRound)) {
      ...
      if (Number.isNaN(roundNum)) {
        this.logger.warn(
          `Invalid round number "${round}" when adding files for stream ${stream} (run ${runId})`,
        );
        continue;
      }
  ...
  async updateMissingOutputs(
    ...
    for (const [round, files] of Object.entries(filesByRound)) {
      ...
      if (Number.isNaN(roundNum)) {
        this.logger.warn(
          `Invalid round number "${round}" when updating missing files for stream ${stream} (run ${runId})`,
        );
        continue;
      }
