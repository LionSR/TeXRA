...
export function updateStreamTabs(streams, activeStream) {
  ...
  if (!activeStream) {
    ...
    const streamStatus = progressViewState.getStreamStatus(activeStream);
...
export function updateStatus(status) {
  ...
  if (status) {
    ...
    if (currentStream && status !== STATUS.READY) {
      progressViewState.setStreamStatus(currentStream, status);
    }
  }
}
...
export function updateUsageSummary(usage) {
  ...
  if (!totals) {
    ...
    for (const group of progressViewState.getLogGroups().values()) {
      if (group.usage) {
...
function propagateUsageToParents(groupId) {
  const group = progressViewState.getLogGroup(groupId);
  if (!group) return;
...
export function updateGroupUsage(groupId, usage, skipPropagate = false) {
  ...
  if (group) {
    ...
    progressViewState.setLogGroup(groupId, group);
  }
  if (!skipPropagate) {
...
function collapseGroupAndChildren(groupId) {
  ...
  progressViewState.setGroupToggleState(groupId, true);

  for (const [childId, group] of progressViewState.getLogGroups()) {
...
function collapsePreviousTopLevelGroup(currentGroupId) {
  const current = progressViewState.getLogGroup(currentGroupId);
  if (!current) return;

  let previousId = null;
  let previousStart = -Infinity;

  for (const [id, group] of progressViewState.getLogGroups()) {
...
export function addLogGroup(group) {
  progressViewState.setLogGroup(group.id, group);
  ...
  const isCollapsed = progressViewState.getGroupToggleState(group.id);
  ...
  detailsElem.addEventListener('toggle', () => {
    progressViewState.setGroupToggleState(group.id, !detailsElem.open);
  });
      ...
      for (let i = 0; i < childElements.length; i++) {
        ...
        // Check if it's another group header
          ...
          if (timeElem) {
            ...
            const otherGroup = progressViewState.getLogGroup(otherGroupId);
...
export function applyGroupToggleStates() {
  const logGroups = progressViewState.getLogGroups();
  for (const [groupId, _] of logGroups) {
    const isCollapsed = progressViewState.getGroupToggleState(groupId);
