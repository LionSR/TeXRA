...
/**
 * Tool-specific condensers for input display.
 * Returns a condensed, human-readable version of the input.
 */
const TOOL_CONDENSERS = {
  ...
  file_op: (inp) =>
    `${inp.operation || 'op'}: ${inp.path || inp.file_path || ''}`,
  ...
  // LaTeX tools
  texcount: (inp) =>
    Array.isArray(inp.files) ? `${inp.files.length} file(s)` : inp.path,
...
export const formatToolInput = (toolName, input) => {
  ...
  const condenser = TOOL_CONDENSERS[toolName];
