...
export class RunSelector {
  ...
  setSessionKind(kind) {
    this._allowDisplay = kind !== 'toolUse';
    this._syncVisibility();
  }
  ...
  _getSortedRuns() {
    return Array.from(this._runs.values()).sort((a, b) => {
      ...
      const safeATime = normalizeTime(a.startTime);
      const safeBTime = normalizeTime(b.startTime);
  ...
  _syncVisibility() {
    ...
    const shouldShow = hasRuns && this._allowDisplay;
    ...
    if (dropdown) {
      ...
      dropdown.setAttribute('aria-hidden', shouldShow ? 'false' : 'true');
    }
