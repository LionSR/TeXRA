...
| `src/platform/`     | The host-port definitions hosts implement: the process-wide `Platform` (`lifecycle`, `agentDirectories`), the per-workspace `WorkspaceRoots` (workspace, storage, config, state), and the `Secrets`/`FileSystem`/`AgentResume`/`LanguageModel` services `installProcessRuntime` provides. Logging is separate (`src/logger/`) |
