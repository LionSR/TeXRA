...
bump-formula-pr`) to the existing `version-bump.yml` / publish job. On release it
  computes the new url + sha256 and opens a PR on the tap. With a tap-scoped PAT it
  can push directly and auto-merge.
