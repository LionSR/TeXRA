...
- 1 file modified: `src/shared/hostBridge.ts` (~45 LOC) — feature-detect Electron, route through `window.electron` instead of `acquireVsCodeApi`. Component code unchanged.
...
- 1 modified file: `src/shared/hostBridge.ts` (~45 LOC) — feature-detect `window.electron`, fall through to `acquireVsCodeApi` otherwise. Keeps both hosts working from one codebase.
