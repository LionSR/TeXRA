...
The stack now provides a working scaffold, packaged `texra run` validation, CLI approval prompts, a structured-logger host-sink migration slice, a plain terminal chat loop, async-scoped tool-call context storage, and the first real split between run-owned and call-owned tool-context views. PR #3847 stores separate run/call views in the async context frame, guards their fields with exhaustive key maps, and migrates clear run-owned and call-owned readers such as path readers, tracker operations, todo state, and plan state. It does not claim to finish every future v1 item in this PRD. In particular, the richer `texra chat` renderer remains tracked by #3848, and the remaining mixed or compatibility tool-context readers remain tracked by #3849.
