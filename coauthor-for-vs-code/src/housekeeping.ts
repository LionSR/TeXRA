...
const EXCLUDED_DIRS = new Set(["Figs", "Figures", "build", "Versions", "versions", "figs", "figures", "Notes"]);
...
function getFilePatterns(base: string, model: string, agent: string, numRounds: number = 4): string[] {
...
async function moveFile(source: string, destination: string): Promise<void> {
    ...
    try {
        ...
        const sourceExists = await vscode.workspace.fs.stat(sourceUri).then(() => true, () => false);
        if (!sourceExists) {
        ...
        outputChannel.appendLine(`[Error] Error moving file from ${source} to ${destination}: ${error}`);
...
async function copyFile(source: string, destination: string): Promise<void> {
    ...
    try {
        ...
        outputChannel.appendLine(`Successfully copied: source=${source} to destination=${destination}`);
    } catch (error) {
        outputChannel.appendLine(`[Error] Error copying file from source=${source} to destination=${destination}: ${error}`);
...
async function findFile(inputDir: string, pattern: string, ext?: string): Promise<string | null> {
    ...
    const searchDirs = [path.join(workspacePath, inputDir, "build"), path.join(workspacePath, inputDir)];
    
    for (const searchDir of searchDirs) {
        try {
            const dirUri = vscode.Uri.file(searchDir);
            
            const exists = await vscode.workspace.fs.stat(dirUri).then(() => true, () => false);
            if (!exists) {
            ...
            outputChannel.appendLine(`Error searching directory searchDir=${searchDir}: ${error}`);
            continue;
        }
    }
    return null;
...
export async function runCleanSingle(model: string, inputFile: string, agent: string): Promise<void> {
    outputChannel.appendLine(`runCleanSingle called with: model=${model}, inputFile=${inputFile}, agent=${agent}`);
    if (!inputFile || !model || !agent) {
        outputChannel.appendLine(`[Error] Missing required parameters: model=${model}, inputFile=${inputFile}, agent=${agent}`);
    ...
    outputChannel.appendLine(`Parsed paths: baseName=${baseName}, inputDir=${inputDir}`);
    ...
    for (const pattern of filePatterns) {
        for (const ext of extensions) {
            ...
            const buildFilePath = path.join(workspacePath, inputDir, 'build', `${pattern}${ext}`);
            ...
            try {
                ...
                const buildExists = await vscode.workspace.fs.stat(buildUri).then(() => true, () => false);
                if (buildExists) {
                    outputChannel.appendLine(`Found build file to delete: ${buildFilePath}`);
                    filesFound = true;
                    await deleteFile(buildFilePath);
                }
            } catch (error) {
                outputChannel.appendLine(`Build file not found or error: ${buildFilePath}`);
            }
        }
    }

    if (!filesFound) {
        outputChannel.appendLine(`No matching files found to clean for ${inputFile}`);
        vscode.window.showInformationMessage(`No files found to clean for ${inputFile}`);
    } else {
...
export async function runPackSingle(
    ...
    outputChannel.appendLine(`runPackSingle called with: model=${model}, inputFile=${inputFile}, agent=${agent}, ${outputFolder}`);
    if (!inputFile || !model || !agent) {
        outputChannel.appendLine(`[ERROR] Missing required parameters: model=${model}, inputFile=${inputFile}, agent=${agent}`);
    ...
    const agentFirstNameChunk = getAgentFirstNameChunk(agent);
    const filePatterns = [...getFilePatterns(baseName, model, agentFirstNameChunk), baseName];
    outputChannel.appendLine(`Generated patterns: ${filePatterns}`);
    ...
    for (const pattern of filePatterns) {
        for (const ext of PACK_EXTENSIONS) {
            ...
            if (filePath) {
                ...
                if (filePath === path.join(workspacePath, inputFile) || pattern === baseName) {
    ...
    if (movedFiles.length > 0 || copiedFiles.length > 0) {
        ...
        outputFolder = outputFolder || path.join(workspacePath, inputDir, "Versions", `${now}_${baseName}_${agent}_${model}`);
...
export async function runCleanBuild(): Promise<void> {
    ...
    async function cleanBuildDir(directory: string) {
        ...
        try {
            ...
            const exists = await vscode.workspace.fs.stat(buildDirUri).then(() => true, () => false);
    ...
    const processDirectory = async (dirPath: string) => {
        try {
            ...
            for (const [name, type] of entries) {
                if (type === vscode.FileType.Directory && !EXCLUDED_DIRS.has(name.toLowerCase())) {
            ...
            outputChannel.appendLine(`[Error] Error processing directory ${dirPath}: ${error}`);
        }
    };
...
export async function runCleanOutput(): Promise<void> {
    ...
    vscode.window.showInformationMessage('Output files cleaned');
}
