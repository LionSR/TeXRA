...
function checkRepetitionDMP(lastResponse, newResponse) {
    ...
    for (var _i = 0, diffs_1 = diffs; _i < diffs_1.length; _i++) {
        var _a = diffs_1[_i], type = _a[0], text = _a[1];
...
function checkRepetitionDifflib(lastResponse, newResponse) {
    var sequenceMatcher = new difflib.SequenceMatcher(null, lastResponse, newResponse);
    var ratio = sequenceMatcher.ratio();
    var match = sequenceMatcher.findLongestMatch(0, lastResponse.length, 0, newResponse.length);
...
// Example usage:
var lastResponse = "This is a test response with some content that might be repeated. Here is a long section that could potentially be duplicated multiple times across responses.";
var newResponse = "This is a test response with some content that might be repeated. Here is a long section that could potentially be duplicated multiple times across responses. And here is some additional content that makes it different.";
