import {
  checkRepetitionDMP,
  checkRepetitionDifflib,
  logMassiveRepetition,
} from './outputUtils';
...
const testCases = [
  ...
  },
  {
    name: 'Mixed content',
    lastResponse:
      'This is a test response with some content that might be repeated. ' +
...
testCases.forEach(({ name, lastResponse, newResponse }) => {
  ...
  if (
    dmpResult.massiveRepetitionDetected ||
    difflibResult.massiveRepetitionDetected
  ) {
