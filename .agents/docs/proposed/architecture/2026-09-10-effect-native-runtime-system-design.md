...
| `parallel()`                  | realm-side `Promise.all` over thunks, cap 512; host side one `PQueue` plus the child loop's own budget                                        | no realm-side fan-out: the script yields `all()` and the host runs it as a fail-fast `Effect.forEach` (amended by the [generator protocol](../../implemented/architecture/2026-09-25-workflow-script-generator-protocol.md); realm isolation holds because the host calls only the trusted `step`); host side keeps **two bounds with two meanings**: the session's detached-turn budget (`Semaphore`, acquired around a detached child's turn only, so an in-band child inherits its parent's slot and cannot deadlock a limit of one) and the script's own `all()` concurrency (unbounded by default; the session budget above bounds what runs) |
