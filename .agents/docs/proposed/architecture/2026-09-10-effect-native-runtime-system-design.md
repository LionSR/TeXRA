...
tags across them, two `Effect.fn` loops driven by `Stream.unfold`, one durable append,
...
**Revision 4 (2026-09-11)** re-verifies the note against main at `047c88cf6e` and the
...
| Fact                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Consequence                                                                                                                                                                                    |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 510 files import `effect` (was 526); 467 `Effect.fn`, 350 `Effect.gen`, 243 `Stream.`, 149 `Deferred.`, 142 `Scope`/`Effect.scoped` (non-test); 18 `Context.Service` tags in 14 files; **two** `ManagedRuntime.make` (`sessionLayer.ts:769`, `webviewSessionLayer.ts:80`, one per host root); one `LayerMap.make` (`sessionLayer.ts:525`); **one** `Semaphore.make` (`memoryFileSystem.ts:302`) plus 15 `makeUnsafe`. `PubSub`, `Context.Reference`: zero.                                                        | The idiom is established. What is missing is the runtime's own program tree and the layers that compose it.                                                                                    |
...
| The Effect spine of a run stops at one line: `runFlowWithLifecycle`'s `runner` is `(handle, lifecycle) => Promise<AgentRuntimeFlowResult>` (`AgentRunLifecycle.ts:464-467`). Below it: `src/agent/node/` (675 LoC), `implementations/flows/**` (8,301 LoC; two files import `effect` only to call `effectRuntime()`), `ModelInvocationNode.ts` (845), `modelHandlers/**` (21,058).                                                                                                                                | The run layer is provided at that line; the code below it is the deletion set.                                                                                                                 |
...
- **In-band delegation awaits; it does not parent.** `Runs.launch` returns
  ...
  `Deferred.await(result)`. A detached child's delivery to its parent is **part of the
...
- **The only `forkDetach` in the runtime is gone.** `AgentRunLifecycle.ts:526` forks the
...
| 4   | `@texra/session/SessionEvents`                                                         | session  | exists; `publishBatch` is the C6 transaction and holds the publication `Semaphore(1)`; publications tracked in a `FiberSet`; `aggregate(id, from): Effect<{ rows, lastCommit }>` and `tail(id, afterCommit): Stream`                                                                                                                                               | `SessionHandle.schedulePublication` + `publications` + `settlePublications` (`:612-631`); the four fire-and-forget publish methods become awaited appends                                                                                                                                              |
...
const runTurn = Effect.fn('toolUse.turn')(function* (s: RunState) {
  ...
  if (s.phase === 'waiting') {
    ...
    if (Option.isNone(batch)) return s; // queue ended: the run is ending
...
const invoke = Effect.fn('model.invoke')(function* (turn: ForegroundTurn) {
  ...
  const attempt = (selected: Selected, attemptId: AttemptId) =>
    Effect.gen(function* () {
  ...
  const automatic = (selected: Selected) =>
    attempt(selected, nextAttemptId()).pipe(
      Effect.retry({
  ...
  while (Either.isLeft(result) && isUserRetryable(result.left)) {
    ...
    result = yield* attempt(yield* Ref.get(run.model), nextAttemptId()).pipe(
      Effect.either,
