...
- **`.agents/docs/archived/architecture/2026-08-15-lifecycle-ownership.md` §2** rules
  ...
  is not proposed.
- **`.agents/docs/archived/simplification/2026-08-03-prd-approval-policy-unification.md` §6** fences
...
- **Blocked on explicit rulings:** 2.D and B4 (2.C was ruled and landed).
...
| 2.C       | **Landed, on the ruling to widen the baseline by one row.** `StreamSnapshotStore.requestEviction` releases a record once its seed and sidecar writes settle and nothing stages it for deletion; `SessionFactApplier` requests it for a finished child stream the host is not presenting (`isStreamPresented`, wired by the CLI adapter and `ProgressBackend`), and the CLI's focus-leave release requests it for the stream it stopped presenting. The store publishes summed usage into the summary mirror on every usage write, and the CLI's artifact read falls back to that mirror for a released record, so the subagent list keeps finished children's token counts. `config/ratchets/store-public-surface-baseline.json` carries the one new row with its reason. One regression test pins release and re-seed.                                            |
