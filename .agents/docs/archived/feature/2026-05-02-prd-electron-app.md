...
> **Superseded (2026-08-26).** Web Awesome's color-scheme adoption inverted this. `@shared/wa/waColorScheme` now installs a `MutationObserver` on `<body>` that derives `wa-light`/`wa-dark` from the host's own `vscode-*` classes and `data-vscode-theme-kind`, with `prefers-color-scheme` as the fallback — so frontends do read both of the signals this paragraph says they don't, and the derivation, not a message, owns the rendered color scheme. `BaseWebviewApp.onThemeChange` no longer touches the DOM: writing `document.body.className = theme` erased the host classes the observer and the `:host-context(.vscode-*)` rules depend on. VS Code sets those body signals itself; the desktop renderer, which is the host, writes them through `applyHostBodyTheme()` in `renderer/main.ts`. The `SET_THEME` message survives only where a value (not a class) is needed — the main view and the desktop diff view.
