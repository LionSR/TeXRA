...
> **Evidence, recorded 2026-09-14.** Measured at `854b36ee69` and reproduced on
> `4d538c5e40`, before the persistence and runtime cutover landed. The harness
> writes the pre-cutover event vocabulary (`run.start` with `executionId`, the
> `status` event, `transcript.entry`), which current main no longer accepts, so
> it does not run unmodified after the cutover as sections 1 and 6 intended. A
> post-cutover comparison first needs its drafts ported to the current
> `run.start` and transcript rows. The numbers below remain the pre-cutover half
> of that comparison.
...
`measure-runtime-baseline.mjs` (in this folder) measured, the machine and datasets it
...
```
node .agents/docs/evidence/2026-09-07-pre-cutover-runtime-baseline/measure-runtime-baseline.mjs
```
...
Two measurement primitives are first uses in this repository's source and
tests: `perf_hooks` `monitorEventLoopDelay` and `os.loadavg`, so there is no
prior convention to match for them. `process.memoryUsage` already appears in
the view-replication evidence probes (`.agents/docs/evidence/2026-09-05-view-replication/`).
...
cross-process part actually happened: it fails the run if it observes no foreign
commits.
...
[plan]: ../../proposed/architecture/2026-09-06-effect-runtime-delivery-plan.md
