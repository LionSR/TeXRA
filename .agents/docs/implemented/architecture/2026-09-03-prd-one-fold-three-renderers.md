...
`SessionEvents`; no new bus emit (CLAUDE.md event-channel rule).
...
(`src/agent/runtime/SessionEvents.ts`), which the applier, the CLI projection, the
...
src/agent/runtime/SessionEvents.ts           rewritten as the durable publisher + transient wake level (7.1)
...
- Facts and lifecycle:
  - `src/agent/runtime/SessionEvents.ts`
