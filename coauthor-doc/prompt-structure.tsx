    ...
    <div className="flex items-center space-x-2">
      {[
        ...
        <React.Fragment key={index}>
          ...
          {index < array.length - 1 && <ArrowRight className="w-4 h-4 text-gray-400" />}
        </React.Fragment>
      ))}
    </div>
  </div>
...
const PromptStructureComplete: React.FC = () => {
  ...
  const getBorderColor = (key: string) => {
    ...
    return activeSection === key ? `border-${component.color}-500 bg-${component.color}-50` : 'border-gray-200';
  };
            ...
            {component.output && (
              <>
                ...
                <div className="w-64 p-4 bg-green-50 rounded-lg border border-green-200">
                  ...
                  <p className="text-sm text-green-700 whitespace-pre-line">{component.output.preview}</p>
                </div>
              </>
            )}
          </div>
