...
- [`2026-07-03-tech-debt-audit.md`](../simplification/2026-07-03-tech-debt-audit.md)
- [`2026-07-03-session-scoped-runtime-architecture.md`](../architecture/2026-07-03-session-scoped-runtime-architecture.md)
...
(`agents/docs/archived/architecture/2026-07-03-session-scoped-runtime-architecture.md`), so a tab whose
...
| Filename-era output grammar (`workflowOutputCopyStem`/`midEraWorkflowOutputStem`/`legacyWorkflowOutputRoundRegex` in `src/shared/constants/workflowOutput.ts`) + `.tex`/`.xml` resume fallback (`runReflectionFlow.ts`) | The resume fallback follows the persisted-run retention window because it only resumes pre-#3082 executions. Workspace readers may retire no earlier than 2027-04-21 after checking for pre-refactor files. `compareCommands.ts` remains a live filename-era writer, so deleting the grammar also requires a replacement naming decision and a dated post-writer horizon in #6984. | pending, D3 (#6984); ownership moved by #8347 |
